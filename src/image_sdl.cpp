...
void Image::setPaletteFromImage(const Image *src)
{
	...
	memcpy(surface->format->palette->colors, src->surface->format->palette->colors, sizeof(SDL_Color) * src->surface->format->palette->ncolors);
}
...
int Image::getPaletteIndex(RGBA color)
{
	...
	for (int i = 0; i < surface->format->palette->ncolors; i++) {
		if ((surface->format->palette->colors[i].r == color.r) && (surface->format->palette->colors[i].g == color.g) && (surface->format->palette->colors[i].b == color.b)) {
			return i;
		}
	}
...
void Image::initializeToBackgroundColor(RGBA backgroundColor)
{
	...
	this->fillRect(0, 0, this->w, this->h, backgroundColor.r, backgroundColor.g, backgroundColor.b, backgroundColor.a);
}
...
{
	int bgColor = SDL_MapRGBA(surface->format, static_cast<Uint8>(backgroundColor.r), static_cast<Uint8>(backgroundColor.g), static_cast<Uint8>(backgroundColor.b), static_cast<Uint8>(backgroundColor.a));
...
void Image::performTransparencyHack(unsigned int colorValue, unsigned int numFrames, unsigned int currentFrameIndex, unsigned int haloWidth, unsigned int haloOpacityIncrementByPixelDistance)
{
	...
	int ox, oy;
	for (std::list<std::pair<unsigned int, unsigned int> >::iterator xy = opaqueXYs.begin(); xy != opaqueXYs.end(); ++xy) {
		ox = xy->first;
		oy = xy->second;
