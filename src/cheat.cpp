...
bool CheatMenuController::keyPressed(int key)
{
	...
	switch (key) {
	case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': screenMessage("Tor %d!\n", key - '0');
	...
	case 'c': collisionOverride = !collisionOverride;
		screenMessage("KOLLISONSTEST %s!\n", collisionOverride ? "AUS" : "EIN");
		break;
	case 'e': screenMessage("AUSR]STUNG!\n");
		...
		break;
	case 'f': screenMessage("VOLLE PUNKTE!\n");
	...
	case 'k': screenMessage("KARMA!\n\n");
		...
		break;
	case 'l': if (c->location->map->isWorldMap()) {
		...
		break;
	case 'm': screenMessage("MIXTUREN!\n");
		for (i = 0; i < SPELL_MAX; i++) {
			c->saveGame->mixtures[i] = 99;
		}
		break;
	case 'o': c->opacity = !c->opacity;
		screenMessage("OPAZIT[T %s!\n", c->opacity ? "EIN" : "AUS");
		break;
	case 'p': if ((c->location->viewMode == VIEW_NORMAL) || (c->location->viewMode == VIEW_DUNGEON)) {
		...
		screenMessage("\nANSICHT!\n");
		break;
	case 'r': screenMessage("REAGENZIEN!\n");
		for (i = 0; i < REAG_MAX; i++) {
			c->saveGame->reagents[i] = 99;
		}
		break;
	case 's': screenMessage("BESCHW\\RE!\n");
		screenMessage("WAS?\n");
		summonCreature(gameGetInput());
		break;
	case 't': if (c->location->map->isWorldMap()) {
			MapCoords coords = c->location->coords;
			static MapTile horse = c->location->map->tileset->getByName("horse")->getId(), ship = c->location->map->tileset->getByName("ship")->getId(), balloon = c->location->map->tileset->getByName("balloon")->getId();
			MapTile *choice;
			Tile *tile;
	...
	case 'x': screenMessage("\nVERLASSE!\n");
		...
		musicMgr->play();
		break;
	case 'y': screenMessage("STEIG AUF!\n");
	...
	case 'z': screenMessage("STEIG AB!\n");
		...
		break;
	case U4_FKEY + 0: case U4_FKEY + 1: case U4_FKEY + 2: case U4_FKEY + 3: case U4_FKEY + 4: case U4_FKEY + 5: case U4_FKEY + 6: case U4_FKEY + 7: screenMessage("STEIGERE %s!\n", getVirtueName(static_cast<Virtue>(key - U4_FKEY)));
		...
		c->stats->update();
		break;
	case U4_ESC: case U4_ENTER: case U4_SPACE: screenMessage("NICHTS!\n");
		break;
	default: valid = false;
		break;
	} // switch
...
{
	switch (key) {
	case U4_UP: case U4_LEFT: case U4_DOWN: case U4_RIGHT: c->windDirection = keyToDirection(key);
		screenMessage("WIND %s!\n", getDirectionName(static_cast<Direction>(c->windDirection)));
		doneWaiting();
		return true;
	case 'l': c->windLock = !c->windLock;
