...
bool CheatMenuController::keyPressed(int key)
{
	...
	case '8':
		...
		if (c->location->map->isWorldMap()) {
			const Coords *moongate =
				moongateGetGateCoordsForPhase(key - '1');
			if (moongate) {
	...
	case 'e':
		...
		for (i = WEAP_HANDS + 1; i < WEAP_MAX; i++) {
			const Weapon *weapon =
				Weapon::get(static_cast<WeaponType>(i));
	...
	case 'g':
	{
		...
		bool found = false;
		for (unsigned p = 0;
		     p < c->location->map->portals.size();
		     p++) {
			MapId destid = c->location->map->portals[p]->destid;
			string destNameLower =
			...
			if (destNameLower.find(dest) != string::npos) {
				screenMessage("\n%s\n",
					      ...
					      ->getName().c_str());
				c->location->coords =
					c->location->map->portals[p]->coords;
				found = true;
				break;
			}
		}
	...
	case 'j':
		...
		for (int m = c->saveGame->members; m < 8; m++) {
			if (c->party->canPersonJoin(
				    c->saveGame->players[m].name, NULL
			    )) {
	...
	case 't':
		if (c->location->map->isWorldMap()) {
			...
			static MapTile
				horse =	c->location->map->tileset
				->getByName("horse")->getId(),
				ship = c->location->map->tileset
				->getByName("ship")->getId(),
				balloon = c->location->map->tileset
				->getByName("balloon")->getId();
			MapTile *choice;
			Tile *tile;
			screenMessage("ERZEUGE TRASNPORT!\nWELCHEN? ");
			// Get the transport of choice
			char transport =
				ReadChoiceController::get("spb \033\015");
			switch (transport) {
			case 's':
				choice = &ship;
				break;
	...
	case U4_FKEY + 7:
		screenMessage("STEIGERE %s!\n",
			      getVirtueName(
				      static_cast<Virtue>(key - U4_FKEY)
			      ));
...
bool WindCmdController::keyPressed(int key)
{
	...
	case U4_RIGHT:
		...
		screenMessage("WIND %s!\n",
			      getDirectionName(
				      static_cast<Direction>(c->windDirection)
			      ));
		doneWaiting();
		return true;
	case 'l':
