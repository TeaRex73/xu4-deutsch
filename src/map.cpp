 ...
 */
Direction MapCoords::pathTo(const MapCoords &c, int valid_directions, bool towards, const Map *map, Direction last) const
{
 ...
 */
Direction MapCoords::pathAway(const MapCoords &c, int valid_directions, const Map *map, Direction last) const
{
...
MapTile *Map::tileAt(const Coords &coords, int withObjects)
{
	...
	if ((withObjects == WITH_OBJECTS) && obj) {
		tile = &obj->getTile();
	} else if ((withObjects == WITH_GROUND_OBJECTS) && obj && obj->getTile().getTileType()->isWalkable()) {
		tile = &obj->getTile();
	}
	return tile;
} // Map::tileAt
...
Creature *Map::moveObjects(MapCoords avatar)
{
	...
	for (unsigned int i = 0; i < objects.size(); i++) {
		...
		if (m) {
			...
			if (((m->getType() == Object::PERSON) && (m->getMovementBehavior() == MOVEMENT_ATTACK_AVATAR)) || ((m->getType() == Object::CREATURE) && m->willAttack())) {
...
int Map::getValidMoves(MapCoords from, MapTile transport, bool wanders)
{
	...
	for (d = DIR_WEST; d <= DIR_SOUTH; d = (Direction)(d + 1)) {
		...
		if (m && !isAvatar) { // some creatures/persons have the same tile as the avatar, so we have to adjust
			...
			if ((ontoAvatar && !m->canMoveOntoPlayer()) || (ontoCreature && ((!m->canMoveOntoCreatures() && !to_m->canMoveOntoCreatures()) || (m->isForceOfNature() && to_m->isForceOfNature())))) {
				continue;
			}
		}
		// avatar movement
		if (isAvatar) {
			...
			// avatar or horseback: check walkable
				...
				if (tile.getTileType()->canWalkOn(d) && (!transport.getTileType()->isHorse() || tile.getTileType()->isCreatureWalkable()) && prev_tile.getTileType()->canWalkOff(d)) {
					retval = DIR_ADD_TO_MASK(d, retval);
				}
			}
...
// }
		...
		// creature movement
			...
			if (tile.getTileType()->isFlyable() && m->flies()) {
				...
				if (isWorldMap()) {
					retval = DIR_ADD_TO_MASK(d, retval);
				} else if (tile.getTileType()->isWalkable() || tile.getTileType()->isSwimable() || tile.getTileType()->isSailable()) {
					retval = DIR_ADD_TO_MASK(d, retval);
				}
			}
			// swimming creatures and sailing creatures
			else if (tile.getTileType()->isSwimable() || tile.getTileType()->isSailable() || tile.getTileType()->isShip()) {
			...
			// ghosts and other incorporeal creatures
				...
				if (!(tile.getTileType()->isSwimable() || tile.getTileType()->isSailable())) {
					retval = DIR_ADD_TO_MASK(d, retval);
				}
			}
			// walking creatures
			else if (m->walks()) {
				if (tile.getTileType()->canWalkOn(d) && prev_tile.getTileType()->canWalkOff(d) && tile.getTileType()->isCreatureWalkable() && (!wanders || tile.getTileType()->willWanderOn())) {
					retval = DIR_ADD_TO_MASK(d, retval);
				}
			}
...
bool Map::fillMonsterTable()
{
	...
	for (i = 0; i < MONSTERTABLE_SIZE; i++) {
		Coords c = monsters[i]->getCoords(), prevc = monsters[i]->getPrevCoords();
