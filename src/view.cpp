...
View::View(int x, int y, int width, int height):x(x), y(y), width(width), height(height), highlighted(false), highlightX(0), highlightY(0), highlightW(0), highlightH(0)
{
	if (screen == NULL) {
...
void View::drawHighlighted()
{
	...
	Image *tmp = Image::create(SCALED(highlightW), SCALED(highlightH), false, Image::HARDWARE); /* was SOFTWARE */
