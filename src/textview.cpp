	...
	:View(x, y, columns * CHAR_WIDTH, rows * CHAR_HEIGHT)
{
	...
	eventHandler->getTimer()->add(&cursorTimer,
				      /*SCR_CYCLE_PER_SECOND*/ 4,
				      this);
}
...
/* depending on the status type, apply colorization to the character */
string TextView::colorizeString(string input,
				ColorFG color,
...
{
	ASSERT(cursorPhase >= 0 && cursorPhase < 4,
