...
static const Spell spells[] = {
	{ "Avacus", GINSENG | GARLIC, CTX_ANY, TRANSPORT_ANY, &spellAwaken, Spell::PARAM_PLAYER, 5 }, { "Blincatus", SILK | MOSS, CTX_WORLDMAP, TRANSPORT_FOOT_OR_HORSE, &spellBlink, Spell::PARAM_DIR, 15 }, { "Curitus", GINSENG | GARLIC, CTX_ANY, TRANSPORT_ANY, &spellCure, Spell::PARAM_PLAYER, 5 }, { "Dispelitus", ASH | GARLIC | PEARL, CTX_ANY, TRANSPORT_ANY, &spellDispel, Spell::PARAM_DIR, 20 }, { "Energitus", ASH | SILK | PEARL, (LocationContext)(CTX_COMBAT | CTX_DUNGEON), TRANSPORT_ANY, &spellEField, Spell::PARAM_TYPEDIR, 10 }, { "Firbalatus", ASH | PEARL, CTX_COMBAT, TRANSPORT_ANY, &spellFireball, Spell::PARAM_DIR, 15 }, { "Gatetus", ASH | PEARL | MANDRAKE, CTX_WORLDMAP, TRANSPORT_FOOT_OR_HORSE, &spellGate, Spell::PARAM_PHASE, 40 }, { "Hilus", GINSENG | SILK, CTX_ANY, TRANSPORT_ANY, &spellHeal, Spell::PARAM_PLAYER, 10 }, { "Isbalatus", PEARL | MANDRAKE, CTX_COMBAT, TRANSPORT_ANY, &spellIceball, Spell::PARAM_DIR, 20 }, { "Jinxus", PEARL | NIGHTSHADE | MANDRAKE, CTX_ANY, TRANSPORT_ANY, &spellJinx, Spell::PARAM_NONE, 30 }, { "Kilutus", PEARL | NIGHTSHADE, CTX_COMBAT, TRANSPORT_ANY, &spellKill, Spell::PARAM_DIR, 25 }, { "Ligatus", ASH, CTX_DUNGEON, TRANSPORT_ANY, &spellLight, Spell::PARAM_NONE, 5 }, { "Missailus", ASH | PEARL, CTX_COMBAT, TRANSPORT_ANY, &spellMMissle, Spell::PARAM_DIR, 5 }, { "Negativus", ASH | GARLIC | MANDRAKE, CTX_ANY, TRANSPORT_ANY, &spellNegate, Spell::PARAM_NONE, 20 }, { "Operatus", ASH | MOSS, CTX_ANY, TRANSPORT_ANY, &spellOpen, Spell::PARAM_NONE, 5 }, { "Protectus", ASH | GINSENG | GARLIC, CTX_ANY, TRANSPORT_ANY, &spellProtect, Spell::PARAM_NONE, 15 }, { "Quicatus", ASH | GINSENG | MOSS, CTX_ANY, TRANSPORT_ANY, &spellQuick, Spell::PARAM_NONE, 20 }, { "Resurrectus", ASH | GINSENG | GARLIC | SILK | MOSS | MANDRAKE, CTX_NON_COMBAT, TRANSPORT_ANY, &spellRez, Spell::PARAM_PLAYER, 45 }, { "Slipitus", SILK | GINSENG, CTX_COMBAT, TRANSPORT_ANY, &spellSleep, Spell::PARAM_NONE, 15 }, { "Tremendus", ASH | MOSS | MANDRAKE, CTX_COMBAT, TRANSPORT_ANY, &spellTremor, Spell::PARAM_NONE, 30 }, { "Undedus", ASH | GARLIC, CTX_COMBAT, TRANSPORT_ANY, &spellUndead, Spell::PARAM_NONE, 15 }, { "Vietus", NIGHTSHADE | MANDRAKE, CTX_NON_COMBAT, TRANSPORT_ANY, &spellView, Spell::PARAM_NONE, 15 }, { "Windatus", ASH | MOSS, CTX_WORLDMAP, TRANSPORT_ANY, &spellWinds, Spell::PARAM_FROMDIR, 10 }, { "Xitatus", ASH | SILK | MOSS, CTX_DUNGEON, TRANSPORT_ANY, &spellXit, Spell::PARAM_NONE, 15 }, { "Ypsapus", SILK | MOSS, CTX_DUNGEON, TRANSPORT_ANY, &spellYup, Spell::PARAM_NONE, 10 }, { "Zidaunus", SILK | MOSS, CTX_DUNGEON, TRANSPORT_ANY, &spellZdown, Spell::PARAM_NONE, 5 }
};
...
bool spellCast(unsigned int spell, int character, int param, SpellCastError *error, bool spellEffect)
{
	...
	*error = spellCheckPrerequisites(spell, character);
	...
	if (spellEffect) {
		...
		time = (int)(17790.4 / settings.spellEffectSpeed * spellMp / MP_OF_LARGEST_SPELL);
...
void spellMagicAttack(const string &tilename, Direction dir, int minDamage, int maxDamage)
{
	...
	vector<Coords> path = gameGetDirectionalActionPath(MASK_DIR(dir), MASK_DIR_ALL, (*party)[controller->getFocus()]->getCoords(), 1, 11, Tile::canAttackOverTile, false);
...
{
	ASSERT(player < 8, "player out of range: %d", player);
	GameController::flashTile(c->party->member(player)->getCoords(), "wisp", 2);
...
static int spellSleep(int unused)
{
	...
	for (i = creatures.begin(); i != creatures.end(); i++) {
		...
		if ((m->getResists() != EFFECT_SLEEP) && (xu4_random(0xFF) >= m->getHp())) {
