...
public: static TileAnimTransform *create(const ConfigElement &config);
...
public: TileAnimInvertTransform(int x, int y, int w, int h);
...
public: TileAnimPixelTransform(int x, int y);
virtual void draw(Image *dest, Tile *tile, MapTile &mapTile);
virtual bool drawsTile() const;
int x, y;
...
public: TileAnimScrollTransform(int increment);
virtual void draw(Image *dest, Tile *tile, MapTile &mapTile);
virtual bool drawsTile() const;
private: int increment, current, lastOffset;
};
/**
...
public: TileAnimScrambleTransform() {}
virtual void draw(Image *dest, Tile *tile, MapTile &mapTile);
virtual bool drawsTile() const;
};
/**
...
public: TileAnimFrameTransform():currentFrame(0) {}
virtual void draw(Image *dest, Tile *tile, MapTile &mapTile);
virtual bool drawsTile() const;
...
public: TileAnimPixelColorTransform(int x, int y, int w, int h);
virtual void draw(Image *dest, Tile *tile, MapTile &mapTile);
virtual bool drawsTile() const;
int x, y, w, h;
RGBA *start, *end;
...
public: typedef std::vector<TileAnimTransform *> TileAnimTransformList;
typedef enum {
	FRAME, DIR
} Type;
...
{
	return animTransforms;   /**< Returns a list of transformations under the context. */
}
virtual ~TileAnimContext() {}
private: TileAnimTransformList animTransforms;
};
/**
...
public: TileAnimFrameContext(int frame);
...
public: TileAnimPlayerDirContext(Direction dir);
virtual bool isInContext(Tile *t, MapTile &mapTile, Direction d);
 ...
 */
class TileAnim {
public: TileAnim(const ConfigElement &conf);
std::string name;
...
public: TileAnimSet(const ConfigElement &conf);
