...
DungeonToken Dungeon::tokenForTile(MapTile tile)
{
	const static std::string tileNames[] = {
		"brick_floor", "up_ladder", "down_ladder", "up_down_ladder", "chest", "unimpl_ceiling_hole", "unimpl_floor_hole", "magic_orb", "ceiling_hole", "fountain", "brick_floor", "dungeon_altar", "dungeon_door", "dungeon_room", "secret_door", "brick_wall", ""
	};
...
void dungeonDrinkFountain()
{
	...
	switch (type) {
	/* plain fountain */
	case FOUNTAIN_NORMAL: screenMessage("\nHmm... Keine Wirkung!\n");
		break;
	/* healing fountain */
	case FOUNTAIN_HEALING: if (c->party->member(player)->heal(HT_FULLHEAL)) {
			screenMessage("\nAhh... Erfrischend!\n");
	} else {
			screenMessage("\nHmm... Keine Wirkung!\n");
	}
		break;
	/* acid fountain */
	case FOUNTAIN_ACID: c->party->member(player)->applyDamage(100); /* 100 damage to drinker */
		screenMessage("\nB{h... Scheu~lich!\n");
		break;
	/* cure fountain */
	case FOUNTAIN_CURE: if (c->party->member(player)->heal(HT_CURE)) {
			screenMessage("\nMmm... K|stlich!\n");
	} else {
			screenMessage("\nHmm... Keine Wirkung!\n");
	}
		break;
	/* poison fountain */
	case FOUNTAIN_POISON: if (c->party->member(player)->getStatus() != STAT_POISONED) {
		...
		break;
	default: ASSERT(0, "Invalid call to dungeonDrinkFountain: no fountain at current location");
	} // switch
} // dungeonDrinkFountain
/**
...
void dungeonTouchOrb()
{
	...
	case MAP_DECEIT: stats = STATSBONUS_INT;
		break;
	case MAP_DESPISE: stats = STATSBONUS_DEX;
		break;
	case MAP_DESTARD: stats = STATSBONUS_STR;
		break;
	case MAP_WRONG: stats = STATSBONUS_INT | STATSBONUS_DEX;
		break;
	case MAP_COVETOUS: stats = STATSBONUS_DEX | STATSBONUS_STR;
		break;
	case MAP_SHAME: stats = STATSBONUS_INT | STATSBONUS_STR;
		break;
	case MAP_HYTHLOTH: stats = STATSBONUS_INT | STATSBONUS_DEX | STATSBONUS_STR;
		break;
	default: break;
	}
	/* give stats bonuses */
...
bool Dungeon::ladderUpAt(MapCoords coords)
{
	...
	if ((tokenAt(coords) == DUNGEON_LADDER_UP) || (tokenAt(coords) == DUNGEON_LADDER_UPDOWN)) {
		return true;
	}
	if (a.size() > 0) {
...
bool Dungeon::ladderDownAt(MapCoords coords)
{
	...
	if ((tokenAt(coords) == DUNGEON_LADDER_DOWN) || (tokenAt(coords) == DUNGEON_LADDER_UPDOWN)) {
		return true;
	}
	if (a.size() > 0) {
		Annotation::List::iterator i;
