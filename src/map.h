...
public:
	...
	int getRelativeDirection(const MapCoords &c,
				 const class Map *map = NULL) const;
	Direction pathTo(const MapCoords &c,
			 int valid_dirs = MASK_DIR_ALL,
			 bool towards = true,
			 const class Map *map = NULL,
			 Direction last = DIR_NONE) const;
	Direction pathAway(const MapCoords &c,
			   int valid_dirs = MASK_DIR_ALL,
			   const class Map *map = NULL,
			   Direction last = DIR_NONE) const;
	int movementDistance(const MapCoords &c,
			     const class Map *map = NULL) const;
	int distance(const MapCoords &c,
		     const class Map *map = NULL) const;
...
public:
	...
	class Object *addObject(MapTile tile,
				MapTile prevTile,
				Coords coords);
	class Object *addObject(Object *obj,
				Coords coords);
	...
	int getValidMoves(MapCoords from,
			  MapTile transport,
			  bool wander = false);
