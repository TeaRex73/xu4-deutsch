...
public: MapCoords(int initx = 0, int inity = 0, int initz = 0):Coords(initx, inity, initz) {}
...
Direction pathTo(const MapCoords &c, int valid_dirs = MASK_DIR_ALL, bool towards = true, const class Map *map = NULL, Direction last = DIR_NONE) const;
Direction pathAway(const MapCoords &c, int valid_dirs = MASK_DIR_ALL, const class Map *map = NULL, Direction last = DIR_NONE) const;
...
class Map {
public: enum Type {
	WORLD, CITY, SHRINE, COMBAT, DUNGEON
};
enum BorderBehavior {
	BORDER_WRAP, BORDER_EXIT2PARENT, BORDER_FIXED
};
class Source {
public: Source() {}
...
int getValidMoves(MapCoords from, MapTile transport, bool wander = false);
