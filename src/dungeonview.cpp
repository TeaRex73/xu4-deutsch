...
{
	if (!instance) {
		instance = new DungeonView(BORDER_WIDTH,
					   BORDER_HEIGHT,
					   VIEWPORT_W,
					   VIEWPORT_H);
	}
...
void DungeonView::display(Context *c, TileView *view)
{
	...
	if (screen3dDungeonViewEnabled) {
		...
		if (c->party->getTorchDuration() > 0) {
			for (y = 3; y >= 0; y--) {
				...
				// y==3 is opaque
				if ((y == 3) &&
				    !tiles.front().getTileType()->isOpaque()) {
					for (int y_obj = frthst_nw_vis;
					     y_obj > y;
					     y_obj--) {
						vector<MapTile>	dts =
							getTiles(y_obj, 0);
	...
	/* 3rd-person perspective */
		...
		static MapTile black =
			c->location->map->tileset->getByName("black")
			->getId();
...
void DungeonView::drawInDungeon(Tile *tile,
				int x_offset,
				int distance,
				...
				bool tiledWall)
{
	...
	if (tiledWall) {
		...
		int y = (int)(
			SCALED((VIEWPORT_H * tileHeight / 2.0)
			       + this->y
			       + y_offset)
			- (scaled->height() / 8.0)
		);
...
int DungeonView::graphicIndex(int xoffset,
			      int distance,
			      Direction orientation,
			      DungeonGraphicType type)
{
	...
	if ((type == DNGGRAPHIC_LADDERUP) && (xoffset == 0)) {
		return 48
			+ (distance * 2)
			+ (DIR_IN_MASK(orientation,
				       MASK_DIR_SOUTH | MASK_DIR_NORTH) ?
			   1 :
			   0);
	}
...
void DungeonView::drawTile(Tile *tile,
			   int x_offset,
			   int distance,
			   Direction orientation)
{
	// Draw the tile to the screen
	DungeonViewer.drawInDungeon(tile,
				    x_offset,
				    distance,
				    orientation,
				    tile->isTiledInDungeon());
}
...
DungeonGraphicType DungeonView::tilesToGraphic(const std::vector<MapTile> &tiles)
{
	...
	static const MapTile updown_ladder =
		c->location->map->tileset->getByName("up_down_ladder")
		->getId();
	/*
