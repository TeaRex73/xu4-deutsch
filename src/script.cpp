...
string &Script::Variable::getString()
{
	return s_val;
}
...
void Script::Variable::setValue(const string &v)
{
	s_val = v;
}
 ...
 */
Script::Script():vendorScriptDoc(NULL), scriptNode(NULL), debug(NULL), state(STATE_UNLOADED), nounName("item"), idPropName("id")
{
...
bool Script::load(const string &filename, const string &baseId, const string &subNodeName, const string &subNodeId)
{
	...
	for (node = root->xmlChildrenNode; node; node = node->next) {
		...
		if (baseId == xmlGetPropAsString(node, "id")) {
			...
			for (child = node->xmlChildrenNode; child; child = child->next) {
				if (xmlNodeIsText(child) || (xmlStrcmp(child->name, (const xmlChar *)subNodeName.c_str()) != 0)) {
					continue;
				}
	...
	if (scriptNode) {
		...
		if (subNodeName.empty()) {
			...
			errorFatal("Couldn't find subscript '%s' where id='%s' in script '%s' in %s", subNodeName.c_str(), subNodeId.c_str(), baseId.c_str(), filename.c_str());
		}
	}
...
Script::ReturnCode Script::execute(xmlNodePtr script, xmlNodePtr currentItem, string *output)
{
	...
	for (; current; current = current->next) {
		...
		/* skip comments */
		else if (current->type == XML_COMMENT_NODE) {} else {
			/**
...
void Script::setTarget(const string &val)
{
	target = val;
}
void Script::setChoices(const string &val)
{
	choices = val;
}
void Script::setVar(const string &name, const string &val)
{
	removeCurrentVariable(name);
	variables[name] = new Variable(val);
}
void Script::setVar(const string &name, int val)
{
	removeCurrentVariable(name);
	variables[name] = new Variable(val);
}
...
string Script::getChoices()
{
	return choices;
}
string Script::getInputName()
{
	return inputName;
}
...
void Script::translate(string *text)
{
	...
	while ((pos = text->find_first_of("{")) < text->length()) {
		...
		string current = item;
		while (true) {
			unsigned int open = current.find_first_of("{"), close = current.find_first_of("}");
...
{
	Script::ReturnCode retval = RET_OK;
	int start = getPropAsInt(current, "start"), end = getPropAsInt(current, "end"),
