...
TileAnimTransform *TileAnimTransform::create(const ConfigElement &conf)
{
	...
	switch (type) {
	case 0: transform = new TileAnimInvertTransform(conf.getInt("x"), conf.getInt("y"), conf.getInt("width"), conf.getInt("height"));
		break;
	case 1:
	{
		transform = new TileAnimPixelTransform(conf.getInt("x"), conf.getInt("y"));
	...
	case 2: transform = new TileAnimScrollTransform(conf.getInt("increment"));
		break;
	case 3: transform = new TileAnimFrameTransform();
		break;
	case 4:
	{
		transform = new TileAnimPixelColorTransform(conf.getInt("x"), conf.getInt("y"), conf.getInt("width"), conf.getInt("height"));
		vector<ConfigElement> children = conf.getChildren();
		for (std::vector<ConfigElement>::iterator i = children.begin(); i != children.end(); i++) {
			if (i->getName() == "color") {
				RGBA *rgba = loadColorFromConf(*i);
	...
	case 5: transform = new TileAnimScrambleTransform();
		break;
	} // switch
...
void TileAnimInvertTransform::draw(Image *dest, Tile *tile, MapTile &mapTile)
{
	...
	tile->getImage()->drawSubRectInvertedOn(dest, x * scale, y * scale, x * scale, (tile->getHeight() * mapTile.frame) + (y * scale), w * scale, h * scale);
}
...
void TileAnimPixelColorTransform::draw(Image *dest, Tile *tile, MapTile &mapTile)
{
	...
	for (int j = y * scale; j < (y * scale) + (h * scale); j++) {
		for (int i = x * scale; i < (x * scale) + (w * scale); i++) {
			...
			if ((pixelAt.r >= start->r) && (pixelAt.r <= end->r) && (pixelAt.g >= start->g) && (pixelAt.g <= end->g) && (pixelAt.b >= start->b) && (pixelAt.b <= end->b)) {
...
TileAnimContext *TileAnimContext::create(const ConfigElement &conf)
{
	...
	switch (type) {
	case FRAME: context = new TileAnimFrameContext(conf.getInt("frame"));
		break;
	case DIR: context = new TileAnimPlayerDirContext(Direction(conf.getEnum("dir", dirEnumStrings)));
		break;
