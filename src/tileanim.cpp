	...
	case 1:
	{
		...
		for (std::vector<ConfigElement>::iterator i = children.begin();
			...
			if (i->getName() == "color") {
				...
				((TileAnimPixelTransform *)transform)->
...
void TileAnimInvertTransform::draw(Image *dest, Tile *tile, MapTile &mapTile)
{
	...
	tile->getImage()->drawSubRectInvertedOn(
		...
		y * scale,
		x * scale,
		(tile->getHeight() * mapTile.frame) + (y * scale),
		w * scale, h * scale
	);
}
...
void TileAnimScrollTransform::draw(Image *dest, Tile *tile, MapTile &mapTile)
{
	...
	tile->getImage()->drawSubRectOn(dest,
					0,
					current,
					0,
					tile->getHeight() * mapTile.frame,
					tile->getWidth(),
...
TileAnimPixelColorTransform::TileAnimPixelColorTransform(int x,
							 int y,
							 int w,
							 int h)
{
...
void TileAnimPixelColorTransform::draw(Image *dest,
				       Tile *tile,
				       MapTile &mapTile)
{
	puts("ColTrans!");
	...
	for (int j = y * scale; j < (y * scale) + (h * scale); j++) {
		for (int i = x * scale; i < (x * scale) + (w * scale); i++) {
			...
			tileImage->getPixel(
				i,
				j + (mapTile.frame * tile->getHeight()),
				pixelAt.r,
				pixelAt.g,
				pixelAt.b,
				pixelAt.a);
...
TileAnimContext *TileAnimContext::create(const ConfigElement &conf)
{
	...
	TileAnimContext::Type type =
		(TileAnimContext::Type)conf.getEnum("type",
						    contextTypeEnumStrings);
...
bool TileAnimFrameContext::isInContext(Tile *t,
				       MapTile &mapTile,
				       Direction dir)
{
...
bool TileAnimPlayerDirContext::isInContext(Tile *t,
					   MapTile &mapTile,
					   Direction d)
{
	return d == dir;
...
void TileAnim::draw(Image *dest, Tile *tile, MapTile &mapTile, Direction dir)
{
	...
	for (c = contexts.begin(); c != contexts.end(); c++) {
		if ((*c)->isInContext(tile, mapTile, dir)) {
			...
			for (t = ctx_transforms.begin();
			     t != ctx_transforms.end();
			     t++) {
				TileAnimTransform *transform = *t;
				if (!transform->random
				    || (xu4_random(100) < transform->random)) {
					if (!transform->drawsTile()
					    && !drawn) {
						tile->getImage()
							->drawSubRectOn(
								dest,
