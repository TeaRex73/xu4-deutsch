...
Dialogue *U4LBDialogueLoader::load(void *source)
{
	...
	*/
	...
	if (c->party->size() > 1) {
		...
		case SEX_MALE: bye = new Response("Er sagt:\nLebe wohl, mein Freund!");
			break;
		case SEX_FEMALE: bye = new Response("Er sagt:\nLebe wohl, meine Freundin!");
			break;
		default: ASSERT(0, "Invalid Sex %d", c->party->member(0)->getSex());
		}
	}
...
Response *lordBritishGetHelp(const DynamicResponse *resp)
{
	...
	if (c->saveGame->moves <= 1000) {
		text = string("Um in diesem feindlichen Lande zu }berleben, mu~t Du dich zuerst selbst kennen! " "Versuche, deine Waffen und deine Magie zu meistern!\n\nSei sehr vorsichtig auf diesen " "deinen ersten Reisen durch Britannia.\n\nBis du dich selbst gut kennst, entferne dich " "nicht weit von der Sicherheit der St{dte!\n");
	} else if (c->saveGame->members == 1) {
		text = string("Reise nicht allein durchs offene Land.\n\nEs gibt viele w}rdige Leute in den " "verschiedenen St{dten, bei denen es weise w{re, sie zu bitten, dich zu begleiten.\n\n" "Baue deine Gruppe auf acht Reisende aus, denn nur ") + string((c->party->member(0)->getSex() == SEX_MALE) ? "ein wahrer Anf}hrer" : "eine wahre Anf}hrerin") + string(" kann die Queste gewinnen!\n");
	} else if (c->saveGame->runes == 0) {
		text = string("Lerne die Pfade der Tugend. Versuche, Zutritt zu den acht Schreinen zu erlangen!\n\n" "Finde die Runen, die man zum Eintritte in jeden Schrein braucht, und lerne jeden " "Gesang, oder \"Mantra\", um Meditationen zu fokussieren.\n\nIn den Schreinen wirst du von den " "Taten lernen, die Tugend oder Laster in deinem Inneren zeigen!\n\nW{hle deinen Pfad weise, denn alle guten " "und b|sen Taten werden erinnert und k|nnen zur}ckkehren, dich aufzuhalten!\n");
	} else if (!partialAvatar) {
		text = string("Besuche h{ufig den Seher Hawkwind und verwende seine Weisheit, um dir zu " "helfen, deine Tugend zu erweisen.\n\nWenn du bereit bist, wird Hawkwind dir raten, " "die Erh|hung zum teilweisen Avatartum in einer Tugend zu erlangen.\n\n" "Versuche, ein Teil-Avatar in allen acht Tugenden zu werden, denn erst dann wirst du " "bereit sein, den Kodex zu suchen!\n");
	} else if (c->saveGame->stones == 0) {
		text = string("Gehe nun in die Tiefen der H|hlen. Nimm dort die 8 farbigen Steine " "von den Altar-Podesten in den G{ngen der H|hlen an dich.\n\nFinde die " "Verwendungen dieser Steine heraus, denn sie k|nnen dir im Abgrunde helfen!\n");
	} else if (!fullAvatar) {
		text = string("Du kommst wirklich sehr gut voran auf dem Pfad zum Avatartume! " "Versuche, die Erh|hung in allen acht Tugenden zu erlangen!\n");
...
Response *lordBritishGetIntro(const DynamicResponse *resp)
{
	...
	if (c->saveGame->lbintro) {
		if (c->saveGame->members == 1) {
			...
			intro->add(string("\n\nDu triffst Lord British.\n\nEr sagt:\nWilkommen\n") + c->party->member(0)->getName() + ", und auch du " + c->party->member(1)->getName() + "!\n");
		} else {
			intro->add(string("\n\nDu triffst Lord British.\n\nEr sagt:\nWilkommen\n") + c->party->member(0)->getName() + " und deine gesch{tzten Abenteurer!\n");
		}
		...
		intro->add(string("\n\nDu triffst Lord British.\n\nEr erhebt sich und sagt: Endlich! ") + c->party->member(0)->getName() + string(", du bist gekommen! Wir haben so eine lange, lange Zeit gewartet...\n\n" "Er setzt sich und sagt: Ein neues Zeitalter liegt auf " "Britannia.\n\nDie Gro~en ]blen Herrscher sind fort, aber unserem Volke " "mangelt es an Richtung und Zweck in ihren Leben...\n\n") + string((c->party->member(0)->getSex() == SEX_MALE) ? "Ein Vork{mpfer" : "Eine Vork{mpferin") + string(" der Tugend wird gebraucht. Du k|nntest ") + string((c->party->member(0)->getSex() == SEX_MALE) ? "dieser Vork{mpfer" : "diese Vork{mpferin") + string(" sein, aber nur " "die Zukunft wird es zeigen.\n\nIch werde dir in jeder Weise beistehen, in der " "ich kann!\n\nDein Begehr:\n?"));
