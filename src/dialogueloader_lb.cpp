...
DialogueLoader *U4LBDialogueLoader::instance =
	DialogueLoader::registerLoader(new U4LBDialogueLoader,
				       "application/x-u4lbtlk");
...
Dialogue *U4LBDialogueLoader::load(void *source)
{
	...
	*/
	...
	/* since the original game files are a bit sketchy on
	   the 'abyss' keyword,
	   let's handle it here just to be safe :). Or not since it's fixed.
	dlg->addKeyword("abgrund",
	                new Response("\n\nEr sagt:\nDer Gro~e Stygische "
				     "Abgrund ist die dunkelste Nische "
	                             "des B|sen, die in Britannia }brig "
				     "ist.\n\nMan sagt, da~ in den tiefsten "
	                             "H|hlungen des Abgrunds die Kammer "
				     "des Codex liegt!\n\nMan sagt auch, "
				     "da~ nur eine Person h|chster Tugend "
				     "diese Kammer betreten darf, so "
				     "jemand wie ein Avatar!!!\n"));
	*/
...
Response *lordBritishGetHelp(const DynamicResponse *resp)
{
	...
	if (c->saveGame->moves <= 1000) {
		text = string("Um in diesem feindlichen Lande zu "
		...
		text = string("Reise nicht allein durchs offene Land.\n\n"
			...
			+ ((c->party->member(0)->getSex() == SEX_MALE) ?
			   string("ein wahrer Anf}hrer") :
			   string("eine wahre Anf}hrerin"))
			+ string(" kann die Queste gewinnen!\n");
	} else if (c->saveGame->runes == 0) {
		text = string("Lerne die Pfade der Tugend. Versuche, "
		...
		text = string("Besuche h{ufig den Seher Hawkwind und "
		...
		text = string("Gehe nun in die Tiefen der H|hlen. Nimm dort "
		...
		text = string("Du kommst wirklich sehr gut voran auf dem "
		...
		text = string("Bevor du den Abgrund betrittst, ben|tigst "
			      "du den Dreiteiligen Schl}ssel, und das Wort "
			      "des Einlasses.\n\n Dann darfst du die Kammer "
			      "des Kodexes der Ultimativen Weisheit "
			      "betreten!\n");
	} else {
		text = string("Du erscheinst nun bereit, die letzte Reise "
...
Response *lordBritishGetIntro(const DynamicResponse *resp)
{
	...
	if (c->saveGame->lbintro) {
		...
		intro->add(string("\n\nLord British erhebt sich und sagt: "
				  "Endlich! ")
			   + c->party->member(0)->getName()
			   + string(", du bist gekommen! Wir haben so eine "
				    ...
				    "...\n\n")
			   + ((c->party->member(0)->getSex() == SEX_MALE) ?
			      string("Ein Vork{mpfer") :
			      string("Eine Vork{mpferin"))
			   + string(" der Tugend wird gebraucht. Du "
				    "k|nntest ")
			   + ((c->party->member(0)->getSex() == SEX_MALE) ?
			      string("dieser Vork{mpfer") :
			      string("diese Vork{mpferin"))
			   + string(" sein, aber nur die Zukunft wird es "
