...
{
	return controllerDone; /**< Returns the current value of the global exit flag */
}
...
TimedEvent::TimedEvent(TimedEvent::Callback cb, int i, void *d):callback(cb), data(d), interval(i), current(0) {}
...
bool ReadDirController::keyPressed(int key)
{
	...
	switch (key) {
	case U4_ESC: case U4_SPACE: case U4_ENTER: value = DIR_NONE;
		doneWaiting();
		return true;
	default: if (valid) {
			value = d;
