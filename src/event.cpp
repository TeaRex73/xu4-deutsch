...
Controller *EventHandler::pushController(Controller *c)
{
	...
	getTimer()->add(&Controller::timerCallback,
			c->getTimerInterval(), c);
	return c;
}
 ...
 */
void TimedEventMgr::add(TimedEvent::Callback callback,
			int interval,
			void *data)
{
	...
	if (pos != string::npos) {
		if (key == U4_BACKSPACE) {
			if (len > 0) {
				...
				value.erase(len - 1, 1);
				if (view) {
					view->textAt(screenX + len - 1,
						     screenY, " ");
					view->setCursorPos(screenX + len - 1,
							   screenY, true);
				} else {
					screenHideCursor();
					screenTextAt(screenX + len - 1,
						     screenY, " ");
					screenSetCursorPos(screenX + len - 1,
							   screenY);
					screenShowCursor();
				}
			}
			...
			value += key;
			if (view) {
				view->textAt(screenX + len,
					     screenY,
					     "%c",
					     key);
			} else {
				screenHideCursor();
				screenTextAt(screenX + len,
					     screenY,
					     "%c",
					     key);
...
string ReadStringController::get(int maxlen,
				 int screenX,
				 int screenY,
				 EventHandler *eh)
{
...
bool ReadChoiceController::keyPressed(int key)
{
	...
	value = key;
	if (choices.empty()
	    || (choices.find_first_of(value) < choices.length())) {
