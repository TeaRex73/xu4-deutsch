...
ImageLoader *FMTOWNSImageLoader::instance_tif =
	ImageLoader::registerLoader(new FMTOWNSImageLoader(510),
				    "image/fmtowns-tif");
...
Image *FMTOWNSImageLoader::load(U4FILE *file, int width, int height, int bpp)
{
	...
	if (rawLen < requiredLength) {
		...
		errorWarning("FMTOWNS Image of size %ld "
			     "does not fit anticipated size %ld",
			     rawLen,
			     requiredLength);
		return NULL;
	}
