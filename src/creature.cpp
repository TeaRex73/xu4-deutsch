...
void Creature::load(const ConfigElement &conf)
{
	...
	static const struct {
		...
		{ "undead", MATTR_UNDEAD }, { "good", MATTR_GOOD }, { "swims", MATTR_WATER }, { "sails", MATTR_WATER }, { "cantattack", MATTR_NONATTACKABLE }, { "camouflage", MATTR_CAMOUFLAGE }, { "wontattack", MATTR_NOATTACK }, { "ambushes", MATTR_AMBUSHES }, { "incorporeal", MATTR_INCORPOREAL }, { "nochest", MATTR_NOCHEST }, { "divides", MATTR_DIVIDES }, { "forceOfNature", MATTR_FORCE_OF_NATURE }
	};
	/* steals="" */
	static const struct {
		const char *name;
		...
		{ "food", MATTR_STEALFOOD }, { "gold", MATTR_STEALGOLD }
	};
	/* casts="" */
	static const struct {
		const char *name;
		...
		{ "sleep", MATTR_CASTS_SLEEP }, { "negate", MATTR_NEGATE }
	};
	/* movement="" */
	static const struct {
		const char *name;
		...
		{ "none", MATTR_STATIONARY }, { "wanders", MATTR_WANDERS }
	};
	...
	static const struct {
		...
		{ "swims", MATTR_SWIMS }, { "sails", MATTR_SAILS }, { "flies", MATTR_FLIES }, { "teleports", MATTR_TELEPORT }, { "canMoveOntoCreatures", MATTR_CANMOVECREATURES }, { "canMoveOntoAvatar", MATTR_CANMOVEAVATAR }
	};
	static const struct {
		const char *name;
		TileEffect effect;
	} effects[] = {
		{ "fire", EFFECT_FIRE }, { "poison", EFFECT_POISONFIELD }, { "sleep", EFFECT_SLEEP }
	};
...
bool Creature::specialEffect()
{
	...
	case STORM_ID:
	{
		...
		for (i = c->location->map->objects.begin(); i != c->location->map->objects.end();) {
			obj = *i;
...
void Creature::act(CombatController *controller)
{
	...
	else if ((ranged != 0) && (xu4_random(4) == 0) && ((rangedhittile != "magic_flash") || (*c->aura != Aura::NEGATE))) {
		action = CA_RANGED;
	}
	...
	case CA_RANGED:
	{
		...
		MapCoords m_coords = getCoords(), p_coords = target->getCoords();
		...
		vector<Coords> path = gameGetDirectionalActionPath(dir, MASK_DIR_ALL, m_coords, 1, 11, &Tile::canAttackOverTile, false);
		bool hit = false;
		for (vector<Coords>::iterator i = path.begin(); i != path.end(); i++) {
...
{
	for (StatusList::const_iterator itr = this->status.begin(); itr != this->status.end(); ++itr) {
...
Creature *Creature::nearestOpponent(int *dist, bool ranged)
{
	...
	for (i = map->objects.begin(); i < map->objects.end(); i++) {
		...
		if ((amPlayer != fightingPlayer) || (jinx && !amPlayer && (*i != this))) {
