...
void CombatController::initDungeonRoom(int room, Direction from)
{
	...
	ASSERT(c->location->prev->context & CTX_DUNGEON, "Error: called initDungeonRoom from non-dungeon context");
	{
		...
		unsigned char *party_x = &dng->rooms[room].party_north_start_x[0], *party_y = &dng->rooms[room].party_north_start_y[0];
...
bool CombatController::setActivePlayer(int player)
{
	...
	if (p && !p->isDisabled()) {
		...
		p->setFocus();
		focus = player;
		screenMessage("\n%s\nW:%s\n\020", uppercase(p->getName()).c_str(), uppercase(p->getWeapon()->getName()).c_str());
...
void CombatController::awardLoot()
{
	...
	if (creature->leavesChest() && ground->isCreatureWalkable() && (!(c->location->context & CTX_DUNGEON) || ground->isDungeonFloor())) {
...
bool CombatController::rangedAttack(const Coords &coords, Creature *attacker)
{
	...
	case EFFECT_POISON: case EFFECT_POISONFIELD:
		...
		if ((xu4_random(2) == 0) && (target->getStatus() != STAT_POISONED)) {
			...
			screenMessage("\n%s\n%cVERGIFTET%c\n", uppercase(target->getName()).c_str(), FG_GREEN, FG_WHITE);
	...
	case EFFECT_SLEEP:
		...
		if (xu4_random(2) == 0) {
			...
			screenMessage("\n%s\n%cBET[UBT%c\n", uppercase(target->getName()).c_str(), FG_PURPLE, FG_WHITE);
			target->putToSleep();
		}
		// else screenMessage("Failed.\n");
		break;
...
void CombatController::finishTurn()
{
	...
	if (!quick || player->isDisabled()) {
		...
		} while (!player || player->isDisabled() || /* dead or sleeping */
...
bool CombatController::keyPressed(int key)
{
	...
	case 's': if (settings.debug && map->isDungeonRoom()) {
			...
			for (i = 0; i < 4; i++) {
				...
				screenMessage("  %.1X%.1X  %.3d %.1X%.1X %.1X%.1X\n", triggers[i].x, triggers[i].y, triggers[i].tile, triggers[i].change_x1, triggers[i].change_y1, triggers[i].change_x2, triggers[i].change_y2);
			}
			screenPrompt();
			valid = false;
	} else {
...
void CombatController::attack()
{
	...
	vector<Coords> path = gameGetDirectionalActionPath(MASK_DIR(dir), MASK_DIR_ALL, attacker->getCoords(), 1, range, weapon->canAttackThroughObjects() ? NULL : &Tile::canAttackOverTile, false);
	...
	if (weapon->loseWhenUsed() || (weapon->loseWhenRanged() && (!foundTarget || (targetDistance > 1)))) {
...
void CombatController::update(Party *party, PartyEvent &event)
{
	if (event.type == PartyEvent::PLAYER_KILLED) {
		screenMessage("\n%c%s\nIST TOT%c\n", FG_RED, uppercase(event.player->getName()).c_str(), FG_WHITE);
	}
}
/**
