...
public:
    ...
    bool canWalkOn(Direction d) const   {return DIR_IN_MASK(d, rule->walkonDirs)!=0;}
    bool canWalkOff(Direction d) const  {return DIR_IN_MASK(d, rule->walkoffDirs)!=0;}
    ...
    bool canLandBalloon() const     {return (rule->mask & MASK_CANLANDBALLOON)!=0;}
    bool isLivingObject() const     {return (rule->mask & MASK_LIVING_THING)!=0;}
    bool isReplacement() const      {return (rule->mask & MASK_REPLACEMENT)!=0;}
    bool isWaterReplacement() const {return (rule->mask & MASK_WATER_REPLACEMENT)!=0;}
    ...
    bool willWanderOn() const {return canWalkOn(DIR_ADVANCE) && !(rule->movementMask & MASK_WONTWANDERON);}
    bool isDungeonWalkable() const;
    bool isDungeonFloor() const;
    bool isSwimable() const         {return (rule->movementMask & MASK_SWIMABLE)!=0;}
    bool isSailable() const         {return (rule->movementMask & MASK_SAILABLE)!=0;}
    bool isWater() const            {return (isSwimable() || isSailable());}
    bool isFlyable() const          {return !(rule->movementMask & MASK_UNFLYABLE);}
    bool isDoor() const             {return (rule->mask & MASK_DOOR)!=0;}
    bool isLockedDoor() const       {return (rule->mask & MASK_LOCKEDDOOR)!=0;}
    bool isChest() const            {return (rule->mask & MASK_CHEST)!=0;}
    bool isShip() const             {return (rule->mask & MASK_SHIP)!=0;}
    bool isPirateShip() const       {return name == "pirate_ship";}
    bool isHorse() const            {return (rule->mask & MASK_HORSE)!=0;}
    bool isBalloon() const          {return (rule->mask & MASK_BALLOON)!=0;}
    bool canDispel() const          {return (rule->mask & MASK_DISPEL)!=0;}
    bool canTalkOver() const        {return (rule->mask & MASK_TALKOVER)!=0;}
