 ...
 */
PartyMember::PartyMember(Party *p, SaveGamePlayerRecord *pr):Creature(tileForClass(pr->klass)), player(pr), party(p)
{
...
{
	return (getStatus() == STAT_GOOD || getStatus() == STAT_POISONED) ? false : true;
}
/**
	...
	for (v = 0; v < VIRT_MAX; v++) {
		if (maxVal[v] == 100) { /* already an avatar */
			if (newKarma[v] < 100) { /* but lost it */
				...
				saveGame->karma[v] = 0; /* return to u4dos compatibility */
			}
		} else {
			saveGame->karma[v] = newKarma[v];
		}
...
CannotJoinError Party::join(string name)
{
	...
	for (i = saveGame->members; i < 8; i++) {
		if (name == saveGame->players[i].name) {
			...
			if ((saveGame->karma[saveGame->players[i].klass] > 0) && (saveGame->karma[saveGame->players[i].klass] < 40)) {
