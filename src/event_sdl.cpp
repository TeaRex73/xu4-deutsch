...
TimedEventMgr::TimedEventMgr(int i):baseInterval(i)
{
	...
	id = static_cast<void *>(SDL_AddTimer(i,
					      &TimedEventMgr::callback,
					      this));
	instances++;
}
...
static void handleMouseMotionEvent(const SDL_Event &event)
{
	...
	MouseArea *area;
	area = eventHandler->mouseAreaForPoint(event.button.x,
					       event.button.y);
	if (area) {
				       ...
				       updateScreenCallback updateScreen)
{
	...
	MouseArea *area = eventHandler->mouseAreaForPoint(event.button.x,
							  event.button.y);
			       ...
			       updateScreenCallback updateScreen)
{
	...
	if (verbose) {
		printf("key event: unicode = %d, sym = %d, mod = %d; "
		       "translated = %d\n",
...
KeyHandler *EventHandler::getKeyHandler() const
{
	...
	KeyHandlerController *khc =
		dynamic_cast<KeyHandlerController *>(controllers.back());
	ASSERT(khc != NULL,
	       "EventHandler::getKeyHandler called when controller "
	       "wasn't a keyhandler");
	if (khc == NULL) {
		return NULL;
	}
