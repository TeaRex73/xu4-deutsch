...
{
	switch (key) {
	case U4_RIGHT_SHIFT: case U4_LEFT_SHIFT: case U4_RIGHT_CTRL: case U4_LEFT_CTRL: case U4_RIGHT_ALT: case U4_LEFT_ALT: case U4_RIGHT_META: case U4_LEFT_META: case U4_TAB: return true;
	default: return false;
	}
}
...
#endif
	if (verbose) {
		printf("key event: unicode = %d, sym = %d, mod = %d; translated = %d\n", event.key.keysym.unicode, event.key.keysym.sym, event.key.keysym.mod, key);
	}
	/* handle the keypress */
	if ((key >= 'a') && (key <= '}')) {
		key = mytoupper(key);
...
void EventHandler::sleep(unsigned int usec)
{
	...
	while (stopUserInput) {
		...
		switch (event.type) {
		default: break;
		case SDL_KEYDOWN: case SDL_KEYUP: case SDL_MOUSEBUTTONDOWN: case SDL_MOUSEBUTTONUP:
			// Discard the event.
			break;
		case SDL_MOUSEMOTION: handleMouseMotionEvent(event);
			break;
		case SDL_ACTIVEEVENT: handleActiveEvent(event, eventHandler->updateScreen);
			break;
		case SDL_USEREVENT: if (event.user.code == 0) {
...
void EventHandler::run()
{
	...
	while (!ended && !controllerDone) {
		...
		switch (event.type) {
		default: break;
		case SDL_KEYDOWN: handleKeyDownEvent(event, getController(), updateScreen);
			break;
		case SDL_MOUSEBUTTONDOWN: handleMouseButtonDownEvent(event, getController(), updateScreen);
			break;
		case SDL_MOUSEMOTION: handleMouseMotionEvent(event);
			break;
		case SDL_USEREVENT: eventHandler->getTimer()->tick();
			break;
