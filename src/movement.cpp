...
void moveAvatarInDungeon(MoveEvent &event)
{
	...
	int advancing = realDir == c->saveGame->orientation, retreating = realDir == dirReverse((Direction)c->saveGame->orientation);
	MapTile *tile;
	...
	if (MAP_IS_OOB(c->location->map, newCoords)) {
		...
		if (sameExit) {
			...
			if (ct->isWinOrLose() && !ct->isCamping()) {
				...
				if (ct->getCreature() && ct->getCreature()->isEvil() && (c->party->member(member)->getHp() == c->party->member(member)->getMaxHp())) {
		...
		if (cm->isDungeonRoom()) {
			...
			for (i = 0; i < 4; i++) {
				...
				if (newCoords == trigger) {
					MapCoords change1(triggers[i].change_x1, triggers[i].change_y1, c->location->coords.z), change2(triggers[i].change_x2, triggers[i].change_y2, c->location->coords.z);
					/**
