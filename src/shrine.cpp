...
void Shrine::enter()
{
	...
	if (settings.enhancements
		...
		screenMessage("DU BETRITTS DEN URALTEN SCHREIN UND "
			      "SETZT DICH VOR DEN ALTAR...\n");
	}
	screenMessage("\n]BER WELCHE TUGEND MEDITIERST DU?\n?");
	string virtue;
	virtue = ReadStringController::get(32,
					   TEXT_AREA_X + c->col,
					   TEXT_AREA_Y + c->line);
	int choice;
	screenMessage("\n\nWIE VIELE\nZYKLEN (0-3)?");
	...
	completedCycles = 0;
	screenMessage("\n\n");
	// ensure the player chose the right virtue and entered
	// a valid number for cycles
	if (cycles == 0) {
	...
	if (strncasecmp(virtue.c_str(), getVirtueName(getVirtue()), 6) != 0) {
		screenMessage("ES GELINGT DIR NICHT, DEINE GEDANKEN AUF "
	...
	if (((c->saveGame->moves / SHRINE_MEDITATION_INTERVAL) >= 0x10000)
		...
		screenMessage("DEIN GEIST IST NOCH ERSCH\\PFT VON "
			      "DEINER LETZTEN MEDITATION!\n");
		eject();
	}
} // Shrine::enter
...
void Shrine::enhancedSequence()
{
	...
	Object *obj = addCreature(creatureMgr->getById(BEGGAR_ID),
...
void Shrine::askMantra()
{
	...
	string mantra;
	mantra = ReadStringController::get(4,
					   TEXT_AREA_X + c->col,
					   TEXT_AREA_Y + c->line);
	screenMessage("\n");
	if (strcasecmp(mantra.c_str(), getMantra().c_str()) != 0) {
		c->party->adjustKarma(KA_BAD_MANTRA);
		screenMessage("\nES GELINGT DIR NICHT, DEINE GEDANKEN "
			      "MIT DIESEM MANTRA ZU FOKUSSIEREN!\n");
		eject();
	} else if (--cycles > 0) {
		completedCycles++;
		...
		if (elevated) {
			screenMessage("\nDU HAST IN DER TUGEND %s "
				      "TEIL-AVATARTUM ERREICHT.\n\n",
				      uppercase(
					      getVirtueName(getVirtue())
				      ).c_str());
			gameSpellEffect(-1, -1, SOUND_MAGIC);
		} else {
			screenMessage("\nDEINE GEDANKEN SIND REIN. DIR "
				      "WIRD EINE VISION ZUTEIL!\n");
		}
