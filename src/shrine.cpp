...
void Shrine::enter()
{
    ...
    if (settings.enhancements
        ...
        screenMessage(
            "DU BETRITTST DEN URALTEN SCHREIN UND SETZT DICH VOR DEN ALTAR..."
            "\n"
        );
    }
    ...
    if (((c->saveGame->moves / SHRINE_MEDITATION_INTERVAL) >= 0x10000)
        ...
        screenMessage(
            "DEIN GEIST IST NOCH ERSCH\\PFT VON DEINER LETZTEN MEDITATION!\n"
        );
        eject();
    }
} // Shrine::enter
...
void Shrine::enhancedSequence()
{
    ...
    Object *obj = addCreature(
        creatureMgr->getById(BEGGAR_ID), Coords(5, 10, c->location->coords.z)
    );
...
void Shrine::showVision(bool elevated)
{
    ...
    if (elevated) {
        ...
        screenMessage(
            "\n%s",
            uppercase(
                shrineAdvice[getVirtue() * 3 + completedCycles - 1]
            ).c_str()
        );
    }
}
