...
public: static U4FILE *open(const string &fname);
virtual void close();
...
public: static U4FILE *open(const string &fname, const U4ZipPackage *package);
virtual void close();
virtual int seek(long offset, int whence);
virtual long tell();
virtual size_t read(void *ptr, size_t size, size_t nmemb);
...
string u4find_path(const string &fname, std::list<string> specificSubPaths)
{
	...
	for (std::list<string>::iterator rootItr = u4Path.rootResourcePaths.begin(); rootItr != u4Path.rootResourcePaths.end() && !f; ++rootItr) {
		for (std::list<string>::iterator subItr = specificSubPaths.begin(); subItr != specificSubPaths.end() && !f; ++subItr) {
