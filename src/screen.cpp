...
enum LayoutType {
	LAYOUT_STANDARD, LAYOUT_GEM, LAYOUT_DUNGEONGEM
};
struct Layout {
	string name;
	LayoutType type;
...
vector<MapTile> screenViewportTile(unsigned int width, unsigned int height, int x, int y, bool &focus)
{
	...
	if ((c->location->map->width <= width) && (c->location->map->height <= height)) {
...
void screenDrawImage(const string &name, int x, int y)
{
	...
	if (subimage) {
		...
		info->image->alphaOn();
		if (info) {
			info->image->drawSubRect(x, y, subimage->x * (settings.scale / info->prescale), subimage->y * (settings.scale / info->prescale), subimage->width * (settings.scale / info->prescale), subimage->height * (settings.scale / info->prescale));
			return;
		}
	}
...
void screenDrawImageInMapArea(const string &name)
{
	...
	info->image->drawSubRect(BORDER_WIDTH * settings.scale, BORDER_HEIGHT * settings.scale, BORDER_WIDTH * settings.scale, BORDER_HEIGHT * settings.scale, VIEWPORT_W * TILE_WIDTH * settings.scale, VIEWPORT_H * TILE_HEIGHT * settings.scale);
}
/**
...
void screenShowChar(int chr, int x, int y)
{
	...
	charsetInfo->image->drawSubRect(x * charsetInfo->image->width(), y * (CHAR_HEIGHT * settings.scale), 0, chr * (CHAR_HEIGHT * settings.scale), charsetInfo->image->width(), CHAR_HEIGHT * settings.scale);
}
/**
...
void screenScrollMessageArea()
{
	...
	screen->drawSubRectOn(screen, TEXT_AREA_X * charsetInfo->image->width(), TEXT_AREA_Y * CHAR_HEIGHT * settings.scale, TEXT_AREA_X * charsetInfo->image->width(), (TEXT_AREA_Y + 1) * CHAR_HEIGHT * settings.scale, TEXT_AREA_W * charsetInfo->image->width(), (TEXT_AREA_H - 1) * CHAR_HEIGHT * settings.scale);
	screen->fillRect(TEXT_AREA_X * charsetInfo->image->width(), TEXT_AREA_Y * CHAR_HEIGHT * settings.scale + (TEXT_AREA_H - 1) * CHAR_HEIGHT * settings.scale, TEXT_AREA_W * charsetInfo->image->width(), CHAR_HEIGHT * settings.scale, 0, 0, 0);
	screenRedrawScreen();
}
void screenCycle()
{
...
void screenUpdateMoons()
{
	...
	/* show the current moons (non-combat) */
		...
		trammelChar = (c->saveGame->trammelphase == 0) ? ' ' : MOON_CHAR + c->saveGame->trammelphase - 1;
		feluccaChar = (c->saveGame->feluccaphase == 0) ? ' ' : MOON_CHAR + c->saveGame->feluccaphase - 1;
...
void screenUpdateWind()
{
	...
	if (c->location->context == CTX_DUNGEON) {
		...
		screenTextAt(WIND_AREA_X, WIND_AREA_Y, "BLICK %4s", uppercase(getDirectionName((Direction)c->saveGame->orientation)).c_str());
	}
	/* show the wind direction */
	else if ((c->location->context & CTX_NON_COMBAT) == c->location->context) {
		screenEraseTextArea(WIND_AREA_X, WIND_AREA_Y, WIND_AREA_W, WIND_AREA_H);
		screenTextAt(WIND_AREA_X, WIND_AREA_Y, "WIND: %4s", uppercase(getDirectionName((Direction)c->windDirection)).c_str());
	}
...
void screenFindLineOfSightDOS(vector<MapTile> viewportTiles[VIEWPORT_W][VIEWPORT_H])
{
	...
	for (x = VIEWPORT_W / 2 - 1; x >= 0; x--) {
		if (screenLos[x + 1][VIEWPORT_H / 2] && !viewportTiles[x + 1][VIEWPORT_H / 2].front().getTileType()->isOpaque()) {
	...
	for (y = VIEWPORT_H / 2 - 1; y >= 0; y--) {
		if (screenLos[VIEWPORT_W / 2][y + 1] && !viewportTiles[VIEWPORT_W / 2][y + 1].front().getTileType()->isOpaque()) {
	...
	for (y = VIEWPORT_H / 2 - 1; y >= 0; y--) {
		for (x = VIEWPORT_W / 2 - 1; x >= 0; x--) {
			if (screenLos[x][y + 1] && !viewportTiles[x][y + 1].front().getTileType()->isOpaque()) {
				screenLos[x][y] = 1;
			} else if (screenLos[x + 1][y] && !viewportTiles[x + 1][y].front().getTileType()->isOpaque()) {
				screenLos[x][y] = 1;
			} else if (screenLos[x + 1][y + 1] && !viewportTiles[x + 1][y + 1].front().getTileType()->isOpaque()) {
				screenLos[x][y] = 1;
			}
		}
		for (x = VIEWPORT_W / 2 + 1; x < VIEWPORT_W; x++) {
...
void screenFindLineOfSightEnhanced(vector<MapTile> viewportTiles[VIEWPORT_W][VIEWPORT_H])
{
	...
	int octant;
	int xOrigin, yOrigin, xSign = 1, ySign = 1, reflect = false, xTile, yTile, xTileOffset, yTileOffset;
...
static int screenPointsOnSameSideOfLine(int x1, int y1, int x2, int y2, double a, double b)
{
	...
	if (((p1 > 0.0) && (p2 > 0.0)) || ((p1 < 0.0) && (p2 < 0.0)) || ((p1 == 0.0) && (p2 == 0.0))) {
		return 1;
	}
	return 0;
}
...
int screenPointInMouseArea(int x, int y, MouseArea *area)
{
	...
	if (area->npoints == 2) {
		if ((x >= (int)(area->point[0].x * settings.scale)) && (y >= (int)(area->point[0].y * settings.scale)) && (x < (int)(area->point[1].x * settings.scale)) && (y < (int)(area->point[1].y * settings.scale))) {
			return 1;
		}
	}
	/* three points define a triangle */
	else if (area->npoints == 3) {
		return screenPointInTriangle(x, y, area->point[0].x * settings.scale, area->point[0].y * settings.scale, area->point[1].x * settings.scale, area->point[1].y * settings.scale, area->point[2].x * settings.scale, area->point[2].y * settings.scale);
	}
	return 0;
}
...
void screenEraseMapArea()
{
	...
	screen->fillRect(BORDER_WIDTH * settings.scale, BORDER_WIDTH * settings.scale, VIEWPORT_W * TILE_WIDTH * settings.scale, VIEWPORT_H * TILE_HEIGHT * settings.scale, 0, 0, 0);
}
void screenEraseTextArea(int x, int y, int width, int height)
{
	Image *screen = imageMgr->get("screen")->image;

	screen->fillRect(x * CHAR_WIDTH * settings.scale, y * CHAR_HEIGHT * settings.scale, width * CHAR_WIDTH * settings.scale, height * CHAR_HEIGHT * settings.scale, 0, 0, 0);
}
/**
...
void screenShake(int iterations)
{
	...
	if (settings.screenShakes) {
		...
		// otherwise image will get messed up because things aren't in their usual places
		screenStill = false;
		...
		for (i = 0; i < iterations; i++) {
			...
			screen->drawSubRectOn(screen, 0, SCALED(shakeOffset), 0, 0, SCALED(320), SCALED(200 - (shakeOffset + 1)), true);
			bottom->drawOn(screen, 0, SCALED(200 - (shakeOffset)), true);
			screen->fillRect(0, 0, SCALED(320), SCALED(shakeOffset), 0, 0, 0, IM_OPAQUE, true);
			screenRedrawScreen();
...
void screenShowGemTile(Layout *layout, Map *map, MapTile &t, bool focus, int x, int y)
{
	...
	if (map->type == Map::DUNGEON) {
		...
		if (charIndex != dungeonTileChars.end()) {
			charsetInfo->image->drawSubRect((layout->viewport.x + (x * layout->tileshape.width)) * settings.scale, (layout->viewport.y + (y * layout->tileshape.height)) * settings.scale, 0, charIndex->second * layout->tileshape.height * settings.scale, layout->tileshape.width * settings.scale, layout->tileshape.height * settings.scale);
		}
	} else {
		...
		if (tile < 128) {
			gemTilesInfo->image->drawSubRect((layout->viewport.x + (x * layout->tileshape.width)) * settings.scale, (layout->viewport.y + (y * layout->tileshape.height)) * settings.scale, 0, tile * layout->tileshape.height * settings.scale, layout->tileshape.width * settings.scale, layout->tileshape.height * settings.scale);
		} else {
			Image *screen = imageMgr->get("screen")->image;
			screen->fillRect((layout->viewport.x + (x * layout->tileshape.width)) * settings.scale, (layout->viewport.y + (y * layout->tileshape.height)) * settings.scale, layout->tileshape.width * settings.scale, layout->tileshape.height * settings.scale, 0, 0, 0);
		}
	}
} // screenShowGemTile
...
void screenGemUpdate()
{
	...
	screen->fillRect(BORDER_WIDTH * settings.scale, BORDER_HEIGHT * settings.scale, VIEWPORT_W * TILE_WIDTH * settings.scale, VIEWPORT_H * TILE_HEIGHT * settings.scale, 0, 0, 0);
	...
	if (c->location->map->type == Map::DUNGEON) {
		...
		while (coordStack.size() > 0) {
			...
			x = currentXY.first;
			y = currentXY.second;
			if ((x < 0) || (x >= layout->viewport.width) || (y < 0) || (y >= layout->viewport.height)) {
			...
			bool focus;
			vector<MapTile> tiles = screenViewportTile(layout->viewport.width, layout->viewport.height, x - center_x + avt_x, y - center_y + avt_y, focus);
			tile = tiles.front();
