		...
		for (y = (src->height() / N) * ii; y < (src->height() / N) * (ii + 1); y++) {
			...
			for (x = 0; x < src->width(); x++) {
				...
				if (colorEqual(a, d) && !colorEqual(b, c)) {
					if ((colorEqual(a, e) && colorEqual(b, l)) || (colorEqual(a, c) && colorEqual(a, f) && !colorEqual(b, e) && colorEqual(b, j))) {
						prod0 = a;
					} else {
						prod0 = colorAverage(a, b);
					}
					if ((colorEqual(a, g) && colorEqual(c, o)) || (colorEqual(a, b) && colorEqual(a, h) && !colorEqual(g, c) && colorEqual(c, m))) {
						prod1 = a;
					} else {
					...
					if ((colorEqual(b, f) && colorEqual(a, h)) || (colorEqual(b, e) && colorEqual(b, d) && !colorEqual(a, f) && colorEqual(a, i))) {
						prod0 = b;
					} else {
						prod0 = colorAverage(a, b);
					}
					if ((colorEqual(c, h) && colorEqual(a, f)) || (colorEqual(c, g) && colorEqual(c, d) && !colorEqual(a, h) && colorEqual(a, i))) {
						prod1 = c;
					} else {
						prod1 = colorAverage(a, c);
					}
