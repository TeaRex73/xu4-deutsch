/*
 * $Id$
 */

#include "vc6.h" // Fixes things if you're using VC6, does nothing otherwise

#include <string>

#include "imageloader_u5.h"

#include "debug.h"
#include "error.h"
#include "image.h"
#include "imageloader.h"
#include "imageloader_u4.h"
#include "lzw/u6decode.h"
#include "u4file.h"

ImageLoader *U5LzwImageLoader::instance =
    registerLoader(new U5LzwImageLoader, "image/x-u5lzw");


/**
 * Loads in the lzw-compressed image and apply the standard U4 16 or
 * 256 color palette.
 */
Image *U5LzwImageLoader::load(
    U4FILE *file, const int width, const int height, const int bpp
)
{
    if (width == -1 || height == -1 || bpp == -1) {
        errorFatal("dimensions not set for u5lzw image");
    }
    U4ASSERT(
        bpp == 4 || bpp == 8 || bpp == 24 || bpp == 32, "invalid bpp: %d", bpp
    );
    const long compressedLen = file->length();
    auto *compressed = new unsigned char[compressedLen];
    file->read(compressed, 1, compressedLen);
    const long rawLen = compressed[0]
        + (compressed[1] << 8)
        + (compressed[2] << 16)
        + (compressed[3] << 24);
    auto *raw = new unsigned char[rawLen];
    U6Decode::lzw_decompress(
        compressed + 4, compressedLen - 4, raw, rawLen
    );
    delete[] compressed;
    if (rawLen != width * height * bpp / 8) {
        delete[] raw;
        return nullptr;
    }
    Image *image = Image::create(
        width,  height, bpp == 4 || bpp == 8, Image::SOFTWARE
    );
    if (!image) {
        delete[] raw;
        return nullptr;
    }
    if (bpp == 8) {
        image->setPalette(U4PaletteLoader::loadVgaPalette(), 256);
    } else if (bpp == 4) {
        image->setPalette(U4PaletteLoader::loadEgaPalette(), 16);
    }
    setFromRawData(image, width, height, bpp, raw);
    delete[] raw;
    return image;
} // U5LzwImageLoader::load
