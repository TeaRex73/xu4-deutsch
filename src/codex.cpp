...
void codexStart()
{
	...
	musicMgr->pause();
	screenMessage("\n\n\n\nPl|tztlich ist es dunkel, und du findest "
		      "dich allein in einer leeren Kammer wieder.\n");
...
static void codexEject(CodexEjectCode code)
{
	...
	case CODEX_EJECT_NO_3_PART_KEY:
		screenMessage("\nDu hast den Dreiteiligen "
			      "Schl}ssel nicht.\n\n");
		break;
	case CODEX_EJECT_NO_FULL_PARTY:
		screenMessage("\nDu hast nicht in allen acht Tugenden "
			      "deine Anf}hrerschaft erwiesen.\n\n");
	...
	case CODEX_EJECT_BAD_INFINITY:
		screenMessage("\nDu kennst die wahre Natur "
			      "des Universums nicht.\n\n");
		break;
	default:
		screenMessage("\nUh-oh, Du bist gerade allzu nahe an "
			      "die L|sung des Spieles herangekommen."
			      "\nB\\SER AVATAR!\n");
		break;
	}
	EventHandler::sleep(2000);
	 ...
	 * that virtue.
	 */
	if ((code >= CODEX_EJECT_HONESTY)
	    && (code <= CODEX_EJECT_HUMILITY)) {
...
static void codexHandleVirtues(const string &virtue)
{
	...
	if ((current < VIRT_MAX)
	    && (strcasecmp(
			...
			deumlaut(getVirtueName(
					 static_cast<Virtue>(current)
					 )).c_str()
		) == 0)) {
		...
		EventHandler::sleep(2000);
		if (current == VIRT_MAX) {
			screenMessage("\nDu bist wohlversiert in den "
				      "Tugenden des Avatars.\n");
	...
	/* answered with the correct base virtue (truth, love, courage) */
		 ...
		 && (strcasecmp(
			     deumlaut(virtue).c_str(),
			     deumlaut(getBaseVirtueName(
					      static_cast<BaseVirtue>(
						      1 << (current - VIRT_MAX)
					      )
				      )).c_str()
		     ) == 0)) {
		...
		if (current < VIRT_MAX + 3) {
			...
			EventHandler::sleep(3000);
			screenEnableCursor();
			screenMessage("\n]ber dem Get|se fragt "
				      "die Stimme:\n");
			eventHandler
				->pushKeyHandler(&codexHandleInfinityAnyKey);
		}
	}
	...
	/* failed 3 times... eject! */
		...
		codexEject(static_cast<CodexEjectCode>(
				   CODEX_EJECT_HONESTY + current
			   ));
		tries = 1;
		current = 0;
	}
...
static bool codexHandleInfinityAnyKey(int key, void *data)
{
	...
	case 1:
		screenMessage("\nWenn alle acht Tugenden des Avatars sich "
			      "vereinen zu, und abgeleitet sind von, den "
			      "Drei Prinzipien der Wahrheit, der Liebe und "
			      "des Mutes...\n");
		codexInfinityIndex++;
		eventHandler->pushKeyHandler(&codexHandleInfinityAnyKey);
		break;
	case 2:
		screenMessage("\nWelches ist dann Das Eine, welches umfa~t, "
			      "und das Ganze ist,\n");
		codexInfinityIndex++;
		eventHandler->pushKeyHandler(&codexHandleInfinityAnyKey);
		break;
	default:
		screenMessage("\naller unabweisbaren Wahrheit, aller "
			      "unendlichen Liebe, und allen unbeugsamen "
			      "Mutes?\n\n");
