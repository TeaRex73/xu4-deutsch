...
static void codexEject(CodexEjectCode code)
{
	struct {
		...
		{ 231, 136 }, { 83, 105 }, { 35, 221 }, { 59, 44 }, { 158, 21 }, { 105, 183 }, { 23, 129 }, { 186, 171 }
	};
	switch (code) {
	case CODEX_EJECT_NO_3_PART_KEY: screenMessage("\nDu hast den Dreiteiligen Schl}ssel nicht.\n\n");
		break;
...
static void codexHandleVirtues(const string &virtue)
{
	...
	if ((current < VIRT_MAX) && (strcasecmp(deumlaut(virtue).c_str(), deumlaut(getVirtueName(static_cast<Virtue>(current))).c_str()) == 0)) {
	...
	else if ((current >= VIRT_MAX) && (strcasecmp(deumlaut(virtue).c_str(), deumlaut(getBaseVirtueName(static_cast<BaseVirtue>(1 << (current - VIRT_MAX)))).c_str()) == 0)) {
		screenDrawImageInMapArea(codexImageNames[current]);
		screenRedrawMapArea();
		current++;
		tries = 1;
