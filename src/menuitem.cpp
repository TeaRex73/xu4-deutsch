	 ...
	 closesMenu(false)
{
	...
	ASSERT(sc == -1 || (sc >= 0 && sc <= (int)text.length()),
...
BoolMenuItem::BoolMenuItem(string text,
			   short x,
			   short y,
			   int shortcutKey,
			   bool *val)
...
BoolMenuItem *BoolMenuItem::setValueStrings(const string &onString,
					    const string &offString)
{
	on = onString;
	off = offString;
	return this;
...
StringMenuItem::StringMenuItem(string text,
			       short x,
			       short y,
			       int shortcutKey,
			       string *val,
...
IntMenuItem::IntMenuItem(string text,
			 short x,
			 short y,
			 int shortcutKey,
			 int *val,
...
string IntMenuItem::getText() const
{
	...
	case MENU_OUTPUT_SHRINE:
		/*
		 * is this code really necessary?  the
		 * increments/decrements can be handled by IntMenuItem(),
		 * as well as the looping once the max is reached. 
		 * more importantly, the minimum value is
		 * inconstant, and based upon another setting that
		 * can be changed independent of this one.
		 * This variable could be set to it's minimum value,
		 * but when the gameCyclesPerSecond setting
		 * is changed, the value of this setting could become
		 * out of bounds.
		 *
		 * settings.shrineTime is only used in one function
		 * within shrine.cpp, and that code appears
		 * to handle the min value, caping the minimum interval at 1.
		 *
		// make sure that the setting we're trying for
		// is even possible
		if (event.getType() == MenuEvent::INCREMENT
		    || event.getType() == MenuEvent::ACTIVATE) {
			...
			if (settingsChanged.shrineTime > MAX_SHRINE_TIME) {
				settingsChanged.shrineTime =
					MEDITATION_MANTRAS_PER_CYCLE /
					settingsChanged.gameCyclesPerSecond;
			} else if (event.getType() == MenuEvent::DECREMENT) {
				settingsChanged.shrineTime--;
				if (settingsChanged.shrineTime
				    < (MEDITATION_MANTRAS_PER_CYCLE /
				       settingsChanged.gameCyclesPerSecond)) {
					settingsChanged.shrineTime =
						MAX_SHRINE_TIME;
