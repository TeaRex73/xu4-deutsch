...
void CampController::begin()
{
	...
	if (settings.campingAlwaysCombat || (xu4_random(8) == 0)) {
		...
		setCreature(
			c->location->prev->map->addCreature(
				m,
	...
	if ((c->location->map->id == 11) && (xu4_random(4) == 0)) {
		...
		if ((city->extraDialogues.size() == 1)
			...
			Coords coords(27,
				      xu4_random(3) + 10,
				      c->location->coords.z);
			// If Isaac is already around, just bring him
			// back to the inn
			for (ObjectDeque::iterator i =
				     c->location->map->objects.begin();
			...
			Person *Isaac;
			Isaac = new Person(
				creatureMgr->getById(GHOST_ID)->getTile()
			);
...
void InnController::maybeAmbush()
{
	...
	if (xu4_random(4) == 0) {
		...
		creature = c->location->map->addCreature(
			creatureMgr->getById(RAT_ID),
			c->location->coords
		);
		screenMessage("\nDU ERWACHST VON EINEM "
			      "SCHAURIGEN GER[USCHE!\n\n");
	} else {
