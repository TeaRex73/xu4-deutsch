...
void osxInit(char *binpath)
{
    ...
    CFURLGetFileSystemRepresentation(
        resourcePath,
        true,
        reinterpret_cast<UInt8 *>(macOSX_AppBundle_Resource_Path),
        MAXPATHLEN
    );
    ...
    if (home && home[0]) {
        dirname = (char *) malloc(
            strlen(home) + strlen(MACOSX_USER_FILES_PATH) + 1
        );
        strcpy(dirname, home);
        ...
        /* Include the application bundle's 'Resources' directory in
           Mac OS X */
    ...
    for (std::list<std::string>::iterator root = mac_roots.begin();
         root != mac_roots.end();
         ++root)
        for (std::list<std::string>::iterator app_supp =
                 mac_app_support.begin();
