...
public:
	...
	void makeBackgroundColorTransparent(int haloSize = 0,
	...
	void putPixel(int x,
		      ...
		      int b,
		      int a,
		      bool anyway = false); // TODO Consider using &
	void putPixelIndex(int x,
			   int y,
			   unsigned int index,
			   bool anyway = false);
	void fillRect(int x,
	 ...
	 * rw, rh.
	 */
	void drawSubRectInverted(int x,
				 int y,
				 int rx,
				 int ry,
				 int rw,
...
/* image drawing methods for drawing onto another image instead of the screen */
	...
	void drawSubRectOn(Image *d,
			   ...
			   int rw,
			   int rh,
			   bool anyway = false) const;
	void drawSubRectInvertedOn(Image *d,
				   int x,
				   int y,
				   int rx,
				   int ry,
