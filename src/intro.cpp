...
public: IntroObjectState():x(0), y(0), tile(0) {}
int x, y;
...
IntroBinData::IntroBinData():sigData(NULL), scriptTable(NULL), baseTileTable(NULL), beastie1FrameTable(NULL), beastie2FrameTable(NULL) {}
...
IntroController::IntroController():Controller(1), backgroundArea(), menuArea(1 * CHAR_WIDTH, 13 * CHAR_HEIGHT, 38, 11), extendedMenuArea(2 * CHAR_WIDTH, 10 * CHAR_HEIGHT, 36, 13), questionArea(INTRO_TEXT_X * CHAR_WIDTH, INTRO_TEXT_Y * CHAR_HEIGHT, INTRO_TEXT_WIDTH, INTRO_TEXT_HEIGHT), mapArea(BORDER_WIDTH, (TILE_HEIGHT * 6) + BORDER_HEIGHT, INTRO_MAP_WIDTH, INTRO_MAP_HEIGHT, "base"), binData(NULL), titles(), // element list
...
void IntroController::drawMapAnimated()
{
	...
	for (i = 0; i < IntroBinData::INTRO_BASETILE_TABLE_SIZE; i++) {
		if (objectStateTable[i].tile != 0) {
			...
			tempMap[objectStateTable[i].x][objectStateTable[i].y] = objectStateTable[i].tile;
		}
	}
	for (y = 0; y < INTRO_MAP_HEIGHT; y++) {
		for (x = 0; x < INTRO_MAP_WIDTH; x++) {
...
void IntroController::drawCard(int pos, int card)
{
	static const char *cardNames[] = {
		"honestycard", "compassioncard", "valorcard", "justicecard", "sacrificecard", "honorcard", "spiritualitycard", "humilitycard"
	};
...
void IntroController::startQuestions(SexType sex)
{
	...
	while (1) {
		...
		questionArea.textAt(0, 2, "%s und %s.", binData->introGypsy[sex == SEX_FEMALE][questionTree[questionRound * 2] + 4].c_str(), binData->introGypsy[sex == SEX_FEMALE][questionTree[questionRound * 2 + 1] + 4].c_str());
...
bool IntroController::doQuestion(int answer)
{
	...
	drawAbacusBeads(questionRound, questionTree[answerInd], questionTree[questionRound * 2 + ((answer) ? 0 : 1)]);
	answerInd++;
	questionRound++;
	if (questionRound > 6) {
		return true;
...
void IntroController::getTitleSourceData()
{
	...
	for (unsigned i = 0; i < titles.size(); i++) {
		if ((titles[i].method != SIGNATURE) && (titles[i].method != BAR)) {
			...
			titles[i].srcImage = Image::create(titles[i].rw * info->prescale, titles[i].rh * info->prescale, false, Image::HARDWARE);
			if (titles[i].srcImage->isIndexed()) {
				titles[i].srcImage->setPaletteFromImage(info->image);
			}
			// get the source image
			info->image->drawSubRectOn(titles[i].srcImage, 0, 0, titles[i].rx * info->prescale, titles[i].ry * info->prescale, titles[i].rw * info->prescale, titles[i].rh * info->prescale);
		}
		...
		case SIGNATURE:
		{
			...
			int blue[16] = {
				255, 250, 226, 226, 210, 194, 161, 161, 129, 97, 97, 64, 64, 32, 32, 0
			};
			int x = 0;
			int y = 0;
			while (srcData[titles[i].animStepMax] != 0) {
				...
				AnimPlot plot = {
					(uint8_t)x, (uint8_t)y, (uint8_t)color.r, (uint8_t)color.g, (uint8_t)color.b, (uint8_t)255
				};
		...
		case MAP:
		{
			...
			titles[i].srcImage->fillRect(8, 8, 304, 80, transparentColor.r, transparentColor.g, transparentColor.b);
		...
		titles[i].destImage = Image::create(2 + (titles[i].prescaled ? SCALED(titles[i].rw) : titles[i].rw) * info->prescale, 2 + (titles[i].prescaled ? SCALED(titles[i].rh) : titles[i].rh) * info->prescale, indexed, Image::HARDWARE);
		if (indexed) {
	...
	Image *scaled = screenScale(info->image, settings.scale / info->prescale, info->image->isIndexed(), 1);
	delete info->image;
	info->image = scaled;
...
bool IntroController::updateTitle()
{
	...
	case BAR:
	{
		...
		while (animStepTarget > title->animStep) {
			...
			title->destImage->fillRect(1, 1, title->animStep, 1, color.r, color.g, color.b);
		}
		break;
	}
	case AND:
	...
	case TITLE:
		...
		for (int i = 0; i < animStepTarget; ++i) {
			title->destImage->putPixel(title->plotData[i].x, title->plotData[i].y, title->plotData[i].r, title->plotData[i].g, title->plotData[i].b, title->plotData[i].a);
		}
		// cover the "present" area with the transparent color
		title->destImage->fillRect(59, 0, 91, 5, transparentColor.r, transparentColor.g, transparentColor.b);
	...
	case SUBTITLE:
	{
		...
		title->srcImage->drawSubRectOn(title->destImage, 1, y + 1, 0, y, title->srcImage->width(), 1 + ((title->animStep - 1) * 2));
		break;
	}
	case MAP:
	{
		...
		// blit src to the canvas one row at a time, center out
		title->srcImage->drawSubRectOn(title->destImage, SCALED(153 - (step * 8)), SCALED(1), 0, 0, SCALED((step + 1) * 8), SCALED(title->srcImage->height()));
		title->srcImage->drawSubRectOn(title->destImage, SCALED(161), SCALED(1), SCALED(312 - (step * 8)), 0, SCALED((step + 1) * 8), SCALED(title->srcImage->height()));
		...
		if (newtime > title->timeDuration + 250 / 4) {
			...
			intro->drawMapStatic();
			screen->drawSubRectOn(title->srcImage, SCALED(8), SCALED(8), SCALED(8), SCALED(13 * 8), SCALED(38 * 8), SCALED(10 * 8));
			title->timeDuration = newtime + 250 / 4;
		}
		title->srcImage->drawSubRectOn(title->destImage, SCALED(161 - (step * 8)), SCALED(9), SCALED(160 - (step * 8)), SCALED(8), SCALED((step * 2) * 8), SCALED((10 * 8)));
		break;
	}
	} // switch
	  // draw the titles
...
void IntroController::drawTitle()
{
	...
	scaled->setTransparentIndex(transparentIndex);
	scaled->drawSubRect(SCALED(title->rx), // dest x, y
	                    SCALED(title->ry), SCALED(1), // src x, y, w, h
	                    SCALED(1), SCALED(title->rw), SCALED(title->rh));
	if (!title->prescaled) {
		delete scaled;
		scaled = NULL;
	}
