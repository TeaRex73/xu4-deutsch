...
public: KeyHandlerController(KeyHandler *handler);
...
public: ReadStringController(int maxlen, int screenX, int screenY, const string &accepted_chars = "abcdefghijklmnopqrstuvwxyz{|}~ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]1234567890 \n\r\010");
...
protected: int maxlen, screenX, screenY;
TextView *view;
string accepted;
};
/**
...
public: ReadIntController(int maxlen, int screenX, int screenY);
...
public: ReadChoiceController(const string &choices);
virtual bool keyPressed(int key);
...
public: WaitController(unsigned int cycles);
virtual bool keyPressed(int key);
...
private: void lock(); /**< Locks the event list */
...
protected: static bool controllerDone;
static bool ended;
TimedEventMgr timer;
