...
static const ItemLocation items[] = {
	{ "Alraune", NULL, "mandrake1", &isReagentInInventory, &putReagentInInventory, NULL, REAG_MANDRAKE, SC_NEWMOONS | SC_REAGENTDELAY }, { "Alraune", NULL, "mandrake2", &isReagentInInventory, &putReagentInInventory, NULL, REAG_MANDRAKE, SC_NEWMOONS | SC_REAGENTDELAY }, { "Schatten", NULL, "nightshade1", &isReagentInInventory, &putReagentInInventory, NULL, REAG_NIGHTSHADE, SC_NEWMOONS | SC_REAGENTDELAY }, { "Schatten", NULL, "nightshade2", &isReagentInInventory, &putReagentInInventory, NULL, REAG_NIGHTSHADE, SC_NEWMOONS | SC_REAGENTDELAY }, { "die Glocke des Mutes", "glocke", "bell", &isItemInInventory, &putItemInInventory, &useBBC, ITEM_BELL, 0 }, { "das Buch der Wahrheit", "buch", "book", &isItemInInventory, &putItemInInventory, &useBBC, ITEM_BOOK, 0 }, { "die Kerze der Liebe", "kerze", "candle", &isItemInInventory, &putItemInInventory, &useBBC, ITEM_CANDLE, 0 }, { "ein Silbernes Horn", "horn", "horn", &isItemInInventory, &putItemInInventory, &useHorn, ITEM_HORN, 0 }, { "das Steuer von Seiner Majest{t Schiff 'Kap'", "steuer", "wheel", &isItemInInventory, &putItemInInventory, &useWheel, ITEM_WHEEL, 0 }, { "den Sch{del Mondains des Zauberers", "sch{del", "skull", &isSkullInInventory, &putItemInInventory, &useSkull, ITEM_SKULL, SC_NEWMOONS }, { "den Roten Stein", "rot", "redstone", &isStoneInInventory, &putStoneInInventory, &useStone, STONE_RED, 0 }, { "den Orangenen Stein", "orange", "orangestone", &isStoneInInventory, &putStoneInInventory, &useStone, STONE_ORANGE, 0 }, { "den Gelben Stein", "gelb", "yellowstone", &isStoneInInventory, &putStoneInInventory, &useStone, STONE_YELLOW, 0 }, { "den Gr}nen Stein", "gr}n", "greenstone", &isStoneInInventory, &putStoneInInventory, &useStone, STONE_GREEN, 0 }, { "den Blauen Stein", "blau", "bluestone", &isStoneInInventory, &putStoneInInventory, &useStone, STONE_BLUE, 0 }, { "den Violetten Stein", "violett", "purplestone", &isStoneInInventory, &putStoneInInventory, &useStone, STONE_PURPLE, 0 }, { "den Schwarzen Stein", "schwarz", "blackstone", &isStoneInInventory, &putStoneInInventory, &useStone, STONE_BLACK, SC_NEWMOONS }, { "den Wei~en Stein", "wei~", "whitestone", &isStoneInInventory, &putStoneInInventory, &useStone, STONE_WHITE, 0 },
	/* handlers for using generic objects */
	{ NULL, "stein", NULL, &isStoneInInventory, NULL, &useStone, -1, 0 }, { NULL, "steine", NULL, &isStoneInInventory, NULL, &useStone, -1, 0 }, { NULL, "schl}ssel", NULL, &isItemInInventory, NULL, &useKey, (ITEM_KEY_C | ITEM_KEY_L | ITEM_KEY_T), 0 }, { NULL, "schl}ssel", NULL, &isItemInInventory, NULL, &useKey, (ITEM_KEY_C | ITEM_KEY_L | ITEM_KEY_T), 0 },
	/* Lycaeum telescope */
	{ NULL, NULL, "telescope", NULL, &useTelescope, NULL, 0, 0 }, { "Mystische R}stung", NULL, "mysticarmor", &isMysticInInventory, &putMysticInInventory, NULL, ARMR_MYSTICROBES, SC_FULLAVATAR }, { "Mystische Schwerter", NULL, "mysticswords", &isMysticInInventory, &putMysticInInventory, NULL, WEAP_MYSTICSWORD, SC_FULLAVATAR }, { "die schwefligen ]berreste einer uralten sosarischen Laserpistole. Sie zerf{llt in deinen Fingern zu Asche", NULL, "lasergun", // lol, where'd that come from?
		                                                                                                                                                                                                                                                                                                                               ...
		                                                                                                                                                                                                                                                                                                                               &isWeaponInInventory, &putWeaponInInventory, 0, 16 }, { "die Rune der Ehrlichkeit", NULL, "honestyrune", &isRuneInInventory, &putRuneInInventory, NULL, RUNE_HONESTY, 0 }, { "die Rune des Mitgef}hls", NULL, "compassionrune", &isRuneInInventory, &putRuneInInventory, NULL, RUNE_COMPASSION, 0 }, { "die Rune der Tapferkeit", NULL, "valorrune", &isRuneInInventory, &putRuneInInventory, NULL, RUNE_VALOR, 0 }, { "die Rune der Gerechtigkeit", NULL, "justicerune", &isRuneInInventory, &putRuneInInventory, NULL, RUNE_JUSTICE, 0 }, { "die Rune des Verzichts", NULL, "sacrificerune", &isRuneInInventory, &putRuneInInventory, NULL, RUNE_SACRIFICE, 0 }, { "die Rune der Ehre", NULL, "honorrune", &isRuneInInventory, &putRuneInInventory, NULL, RUNE_HONOR, 0 }, { "die Rune der Spiritualit{t", NULL, "spiritualityrune", &isRuneInInventory, &putRuneInInventory, NULL, RUNE_SPIRITUALITY, 0 }, { "die Rune der Demut", NULL, "humilityrune", &isRuneInInventory, &putRuneInInventory, NULL, RUNE_HUMILITY, 0 }
};
...
void useStone(int item)
{
	 ...
	 */
	else if ((c->location->map->id == MAP_ABYSS) && (c->location->context & CTX_DUNGEON) && (dynamic_cast<Dungeon *>(c->location->map)->currentToken() == DUNGEON_ALTAR)) {
	 ...
	 */
	else if ((c->location->context & CTX_ALTAR_ROOM) && (coords.x == 5) && (coords.y == 5)) {
		needStoneNames = 4;
...
bool itemConditionsMet(unsigned char conditions)
{
	...
	if ((conditions & SC_NEWMOONS) && !((c->saveGame->trammelphase == 0) && (c->saveGame->feluccaphase == 0))) {
		return false;
	}
	...
	if ((conditions & SC_REAGENTDELAY) && ((c->saveGame->moves & 0xF0) == c->saveGame->lastreagent)) {
		return false;
	}
	return true;
}
...
const ItemLocation *itemAtLocation(const Map *map, const Coords &coords)
{
	...
	for (i = 0; i < N_ITEMS; i++) {
		...
		if ((map->getLabel(items[i].locationLabel) == coords) && itemConditionsMet(items[i].conditions)) {
			return &(items[i]);
		}
	}
	return NULL;
...
void itemUse(const string &shortname)
{
	...
	for (i = 0; i < N_ITEMS; i++) {
		if (items[i].shortname && (strcasecmp(deumlaut(items[i].shortname).c_str(), deumlaut(shortname).c_str()) == 0)) {
			item = &items[i];
...
void itemHandleStones(const string &color)
{
	...
	for (int i = 0; i < 8; i++) {
		if ((strcasecmp(deumlaut(color).c_str(), deumlaut(getStoneName((Virtue)i)).c_str()) == 0) && isStoneInInventory(1 << i)) {
			found = true;
			itemUse(color.c_str());
		}
	}
