 ...
 */
class Annotation {
public: typedef std::list<Annotation> List;
Annotation(const Coords &coords, MapTile tile, bool visual = false, bool coverUp = false);
void debug_output() const;
// Getters
const Coords &getCoords() const
{
	return coords;    /**< Returns the coordinates of the annotation */
}
MapTile &getTile()
{
...
{
	return visual;    /**< Returns true for visual-only annotations */
}
int getTTL() const
{
	return ttl;       /**< Returns the number of turns the annotation has left to live */
}
bool isCoverUp()
{
	return coverUp;
...
{
	coords = c;       /**< Sets the coordinates for the annotation */
}
void setTile(const MapTile &t)
{
	tile = t;         /**< Sets the tile for the annotation */
}
void setVisualOnly(bool v)
{
	visual = v;       /**< Sets whether or not the annotation is visual-only */
}
void setTTL(int turns)
{
	ttl = turns;      /**< Sets the number of turns the annotation will live */
}
void passTurn()
{
	if (ttl > 0) {
...
int              size();
private: Annotation::List annotations;
