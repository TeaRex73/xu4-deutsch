...
static void xmlToTlk(xmlDocPtr doc, FILE *tlk); // NOLINT(misc-include-cleaner)
static xmlDocPtr tlkToXml(FILE *tlk);

static char *my_strdup(const char *s)
{
    char *ret = strdup(s); // NOLINT(misc-include-cleaner)
    if (!ret) {
...
)
{
    ...
    while (*begin) {
        ...
        if (!end) {
            end = begin + strlen(begin); // NOLINT(misc-include-cleaner)
        }
    ...
    for (person = root->children; person; person = person->next) {
        ...
        for (node = person->children; node; node = node->next) {
            ...
            if (strcmp((char *) node->name, "description") == 0) {
                ...
                if (strcasecmp(query, "job") == 0) {
                    ...
                    if (kw > KEYWORD2) {
                        fprintf( // NOLINT(misc-include-cleaner)
                            stderr,
                            ".tlk files only allow 2 topics other than 'job' "
                            "and 'health'\n"
                        );
                        fprintf(
                            stderr,
                            "If you need more than 2 keywords, then it is "
                            "recommended you\n"
                        );
                ...
                fprintf(
                    stderr,
                    "An unknown node %s was found in the .xml file",
                    (char *)node->name
                );
                exit(1);
            }
        ...
        for (i = 0; i < MAX; i++) {
            if (!str[i]) {
                /* FIXME: give better info than the index */
            ...
            if (*str[i] == '\0') {
                free(str[i]); // NOLINT(misc-include-cleaner)
            } else {
                xmlFree(str[i]);
            }
        }
...
static xmlDocPtr tlkToXml(FILE *tlk)
{
    ...
    for (i = 0; ; i++) {
        ...
        addAsText(
            doc,
            xmlNewTextChild(node, NULL, (const xmlChar *)"description", NULL),
            ptr
        );
        ptr += strlen(ptr) + 1;
        job = addAsText(
            doc,
            xmlNewTextChild(node, NULL, (const xmlChar *)"topic", NULL),
            ptr
        );
        ...
        if (target) {
            ...
            q = addAsText(
                doc,
                xmlNewTextChild(
                    target, NULL, (const xmlChar *)"question", NULL
                ),
                question
            );
            ...
            addAsText(
                doc,
                xmlNewTextChild(q, NULL, (const xmlChar *)"yes", NULL),
                yes
            );
            addAsText(
                doc,
                xmlNewTextChild(q, NULL, (const xmlChar *)"no", NULL),
                no
            );
        }
        free(response1);
...
int main(const int argc, char **argv)
{
    ...
    if (argc != 4) {
        fprintf(
            stderr,
            "usage: tlkconv --toxml file.tlk file.xml\n"
            "       tlkconv --fromxml file.xml file.tlk\n"
        );
        exit(1);
    }
    in = fopen(argv[2], "rb"); // NOLINT(misc-include-cleaner)
    if (!in) {
        perror(argv[2]);
        exit(1);
    }
    ...
    if (strcmp(argv[1], "--toxml") == 0) {
        ...
        fseek(in, 0L, SEEK_END); // NOLINT(misc-include-cleaner)
        xmlSize = (int)ftell(in); // NOLINT(misc-include-cleaner)
        fseek(in, 0L, SEEK_SET);
        xml = (char *) malloc(xmlSize); // NOLINT(misc-include-cleaner)
        if (!xml) {
    ...
    fclose(in); // NOLINT(misc-include-cleaner)
    fclose(out);
    return 0;
}
