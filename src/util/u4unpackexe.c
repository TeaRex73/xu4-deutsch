...
int main(int argc, char *argv[])
{
    ...
    if (infileSize < BASE_HEADER_SIZE) {
        printf(
            "Not enough space in %s for %d-byte base header!\n",
            argv[1],
            BASE_HEADER_SIZE
        );
        free(buffer);
    ...
    /* todo: check if the unknown unpacker is actually there, not just if
       there's enough space for it */
    if (headerSize + oldEntrySeg*0x10 + oldEntryOfs + FAR_RET_SIZE
        > infileSize) {
        printf(
            "Not enough space in %s for %d-byte code section!\n",
            argv[1],
            FAR_RET_SIZE
        );
        free(buffer);
    ...
    newStackOfs =
        getWord(buffer + headerSize + oldEntrySeg*0x10 + oldEntryOfs + NEW_SP);
    newEntrySeg = newStackSeg - getWord(
        buffer + headerSize + oldEntrySeg*0x10 + oldEntryOfs + NEW_SEG2
    );
    newEntryOfs = getWord(
        buffer + headerSize + oldEntrySeg*0x10 + oldEntryOfs + NEW_IP
    );
