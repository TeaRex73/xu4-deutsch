...
bool GameController::keyPressed(int key)
{
	...
	if ((key == U4_ENTER) && settings.enhancements && settings.enhancementsOptions.smartEnterKey) {
		...
		if (c->transportContext == TRANSPORT_FOOT) {
			...
			if (obj && (obj->getTile().getTileType()->isShip() || obj->getTile().getTileType()->isHorse() || obj->getTile().getTileType()->isBalloon())) {
				key = 'l';
			}
		}
		...
		if (c->location->context == CTX_DUNGEON) {
			...
			if (up && down) {
				key = 'q';         // This is consistent with the previous code. Ideally, I would have a UI here as well.
			} else if (up) {
				key = 'q';
			} else {
				key = 'y';
	...
	if ((c->location->context & CTX_DUNGEON) && strchr("albkdp|usgh", key)) {
		...
		case U4_UP: case U4_DOWN: case U4_LEFT: case U4_RIGHT:
		{
			...
			if (retval & (MOVE_SUCCEEDED | MOVE_SLOWED) && (c->transportContext == TRANSPORT_HORSE) && c->horseSpeed) {
...
int gameGetPlayer(bool canBeDisabled, bool canBeActivePlayer, bool zeroIsValid)
{
	...
	ASSERT((player == 8) || (player < c->party->size()), "player %d, but only %d members\n", player, c->party->size());
	return player;
} // gameGetPlayer
...
void destroy()
{
	...
	vector<Coords> path = gameGetDirectionalActionPath(MASK_DIR(dir), MASK_DIR_ALL, c->location->coords, 1, 1, NULL, true);
...
bool attackAt(const Coords &coords)
{
	...
	if (!ground->isChest()) {
		...
		if ((under = c->location->map->objectAt(c->location->coords)) && under->getTile().getTileType()->isShip()) {
...
void castSpell(int player)
{
	...
	screenMessage("%s!\n", uppercase(spellGetName(spell)).c_str());                          // Prints spell name at prompt
	c->stats->setView(STATS_PARTY_OVERVIEW);
...
void fire()
{
	...
	vector<Coords> path = gameGetDirectionalActionPath(MASK_DIR(dir), broadsidesDirs, c->location->coords, 1, 3, NULL, false);
	for (vector<Coords>::iterator i = path.begin(); i != path.end(); i++) {
		if (fireAt(*i, true)) {
			return;
		}
...
bool fireAt(const Coords &coords, bool originAvatar)
{
	...
	else if (obj && (obj->getType() == Object::UNKNOWN) && !(obj->getTile().getTileType()->isBalloon() && originAvatar)) {
		validObject = true;
	}
...
bool getChestTrapHandler(int player)
{
	...
	if (passTest) {
		...
		//
		if ((player >= 0) && (c->saveGame->players[player].dex + 25 < xu4_random(100))) {
...
{
	int trammelphase = c->saveGame->trammelphase, feluccaphase = c->saveGame->feluccaphase;
	...
	c->moonPhase = 0;
	while ((c->saveGame->trammelphase != trammelphase) || (c->saveGame->feluccaphase != feluccaphase)) {
		updateMoons(false);
	}
}
/**
...
void GameController::avatarMoved(MoveEvent &event)
{
	...
	if ((c->transportContext & TRANSPORT_FOOT_OR_HORSE) && !(event.result & (MOVE_SLOWED | MOVE_BLOCKED))) {
		if (checkMoongates()) {
	...
	if (!settings.filterMoveMessages) {
		if (event.userEvent) {
			...
			/* show 'Advance' or 'Retreat' in dungeons */
				...
				screenMessage("%s\n", realDir == c->saveGame->orientation ? "Vormarsch" : "R}ckzug");
			}
		}
		if (event.result & MOVE_BLOCKED) {
			soundPlay(SOUND_BLOCKED, false);
...
void readyWeapon(int player)
{
	...
	screenMessage("\nWAFFE-");
	WeaponType weapon = (WeaponType)AlphaActionController::get(WEAP_MAX + 'a' - 1, "WAFFE-");
	c->stats->setView(STATS_PARTY_OVERVIEW);
	if (weapon == -1) {
		return;
	}
...
void talk()
{
	...
	vector<Coords> path = gameGetDirectionalActionPath(MASK_DIR(dir), MASK_DIR_ALL, c->location->coords, 1, 2, &Tile::canTalkOverTile, true);
	for (vector<Coords>::iterator i = path.begin(); i != path.end(); i++) {
		if (talkAt(*i)) {
			return;
		}
	...
	while (!done) {
		...
		if (!found) {
			...
			if (c->saveGame->mixtures[spell] == 99) {
				screenMessage("\n%cDU KANNST NICHT NOCH MEHR VON DIESEM ZAUBER MISCHEN!%c\n", FG_GREY, FG_WHITE);
				break;
			}
...
bool talkAt(const Coords &coords)
{
	...
	if ((talker->getNpcType() == NPC_LORD_BRITISH) && (c->party->member(0)->getStatus() == STAT_DEAD)) {
		c->willPassTurn = false;
		screenMessage("%s, DU SOLLST WIEDER LEBEN!\n", uppercase(c->party->member(0)->getName()).c_str());
...
void talkRunConversation(Conversation &conv, Person *talker, bool showPrompt)
{
	...
	while (conv.state != Conversation::DONE) {
		...
		if (showPrompt) {
			...
			if (!prompt.empty()) {
				if ((conv.state == Conversation::ASK) || (conv.state == Conversation::CONFIRMATION) || (linesused + linecount(prompt, TEXT_AREA_W) > TEXT_AREA_H)) {
					ReadChoiceController::get("");
				}
...
void wearArmor(int player)
{
	...
	screenMessage("\nR]STUNG-");
	ArmorType armor = (ArmorType)AlphaActionController::get(ARMR_MAX + 'a' - 1, "R}STUNG-");
	c->stats->setView(STATS_PARTY_OVERVIEW);
	if (armor == -1) {
		return;
	}
...
void GameController::timerFired()
{
	...
	if (!paused && !pausedTimer) {
		...
		if ((c->transportContext == TRANSPORT_BALLOON) && c->party->isFlying()) {
		 ...
		 */
		Controller *controller = eventHandler->getController();
		if (c->willPassTurn && (controller != NULL) && ((eventHandler->getController() == game) || (dynamic_cast<CombatController *>(eventHandler->getController()) != NULL)) && (gameTimeSinceLastCommand() > 20)) {
			/* pass the turn, and redraw the text area so the prompt is shown */
...
void gameCheckHullIntegrity()
{
	...
	if (!collisionOverride && (c->transportContext == TRANSPORT_FOOT) && c->location->map->tileTypeAt(c->location->coords, WITHOUT_OBJECTS)->isSailable() && !c->location->map->tileTypeAt(c->location->coords, WITH_GROUND_OBJECTS)->isShip() && !c->location->map->getValidMoves(c->location->coords, c->party->getTransport())) {
...
void GameController::checkSpecialCreatures(Direction dir)
{
	 ...
	 * ships
	 */
	if ((dir == DIR_EAST) && (c->location->coords.x == 0xdd) && (c->location->coords.y == 0xe0)) {
		for (i = 0; i < 8; i++) {
	...
	*/
	if ((dir == DIR_SOUTH) && (c->location->coords.x >= 229) && (c->location->coords.x < 234) && (c->location->coords.y >= 212) && (c->location->coords.y < 217) && (*c->aura != Aura::HORN)) {
		for (i = 0; i < 8; i++) {
...
void gameFixupObjects(Map *map)
{
	...
	for (i = 0; i < MONSTERTABLE_SIZE; i++) {
		...
		if (monster->prevTile != 0) {
			...
			MapTile tile = TileMap::get("base")->translate(monster->tile), oldTile = TileMap::get("base")->translate(monster->prevTile);
...
// int attackdelay = MAX_BATTLE_SPEED - settings.battleSpeed;
	...
	MapTile tile(c->location->map->tileset->getByName((m && !m->getWorldrangedtile().empty()) ? m->getWorldrangedtile() : "hit_flash")->getId());
	GameController::flashTile(coords, tile, 1);
	...
	// Destroy objects that were hit
		...
		if (((obj->getType() == Object::CREATURE) && m->isAttackable()) || (obj->getType() == Object::UNKNOWN)) {
			GameController::flashTile(coords, tile, 3);
			c->location->map->removeObject(obj);
			return true;
		}
...
vector<Coords> gameGetDirectionalActionPath(int dirmask, int validDirections, const Coords &origin, int minDistance, int maxDistance, bool (*blockedPredicate)(const Tile *tile), bool includeBlocked)
{
	...
	if (((dirx <= 0) || DIR_IN_MASK(dirx, validDirections)) && ((diry <= 0) || DIR_IN_MASK(diry, validDirections))) {
		for (int distance = 0; distance <= maxDistance; distance++, t_c.move(dirx, c->location->map), t_c.move(diry, c->location->map)) {
			if (distance >= minDistance) {
				...
				if (!includeBlocked && blockedPredicate && !(*(blockedPredicate))(tile)) {
					break;
				}
				path.push_back(t_c);
...
void GameController::creatureCleanup()
{
	...
	for (i = map->objects.begin(); i != map->objects.end();) {
		...
		if ((obj->getType() == Object::CREATURE) && (o_coords.z == c->location->coords.z) && (o_coords.distance(c->location->coords, c->location->map) > MAX_CREATURE_DISTANCE)) {
...
void GameController::checkBridgeTrolls()
{
	...
	if (!c->location->map->isWorldMap() || (c->location->map->tileAt(c->location->coords, WITH_OBJECTS)->id != bridge->getId()) || (xu4_random(8) != 0)) {
		return;
	}
	screenMessage("\nBR]CKENTROLLE!\n");
	...
	if (c->location->context & CTX_DUNGEON) {
		...
		while (!ok && (tries < MAX_TRIES)) {
			...
			if (m) {
				...
				if ((m->sails() && tile->isSailable()) || (m->swims() && tile->isSwimable()) || (m->walks() && tile->isCreatureWalkable() && tile->willWanderOn()) || (m->flies() && tile->isFlyable())) {
					ok = true;
				} else {
					tries++;
				}
...
/**
 * Creates the balloon near Hythloth, but only if the balloon doesn't already exist somewhere
 */
...
void mixReagentsSuper()
{
	...
	while (!done) {
		...
		if ((spell < 'a') || (spell > 'z')) {
			...
			screenMessage("DU KANNST %d MISCHEN.\n", (mixQty > ingQty) ? ingQty : mixQty);
			screenMessage("WIE VIELE? ");
			int howmany = ReadIntController::get(2, TEXT_AREA_X + c->col, TEXT_AREA_Y + c->line);
			if (howmany == 0) {
				screenMessage("\nKEINE GEMISCHT!\n");
